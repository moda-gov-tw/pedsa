...
from . import task_multipleHash,task_multipleHash_new,task_multipleHash_identifier,task_check_identifier,task_resend
...
@app.route("/direct_enc_identifier_async", methods=["POST"])
@swag_from("swagger_yml/MultipleHash_identifier.yml")
def direct_enc_identifier_async():
    log_time = GetLogTime('direct_enc_identifier_async')
    log_time.printLog("Start direct_enc_identifier_async")
    ...
    try:
        ...
        service_ip = data_raw['service_ip']
        # hashkey = "AAAAAABC0DCB39FE182FAF7CE960A2B0BA63AFEEDC76D8A92AED52938AA06ABA"
        # AES_hashkey = "AAAAAABC0DCB39FE182FAF7CE960A2B0BA63AFEEDC76D8A92AED52938AA06ABA"
        identifier = data_raw['identifier']
   ...
   # # "project_name":"運動",
   # # "project_folder":"test0103",
   # # "pets_service_ip":"127.0.0.1" 


    if identifier == '' :
        response = dict()
        errMsg = 'request_error: ' + str('identifier Empty parameter')
        errMsg = errMsg + ' ; request: {}'.format(request)
        response['status'] = -1
        response['errMsg'] = errMsg
        log_time.printLog(errMsg)
    ...
    try: 
        response = dict()
        sqlStr = "SELECT * FROM `PetsService`.`T_Pets_MemberProjectRole` where key_code like '{}' ;".format(identifier)
         
        print("in check_identifier_code: sqlStr="+sqlStr)
        resultCheck = check_conn.doSqlCommand(sqlStr)

        if resultCheck["fetchall"] == [] or len(resultCheck["fetchall"])==0:
            print("no")
            # response['check1'] = "no"
        else:
            # response['check1'] = "yes"

            T_Pets_MemberProjectRole_project_id = resultCheck["fetchall"][0]['project_id']
            T_Pets_MemberProjectRole_member_id = resultCheck["fetchall"][0]['member_id']
            # response['project_id'] = T_Pets_MemberProjectRole_project_id
            # response['member_id'] = T_Pets_MemberProjectRole_member_id

#######################################
        sqlStr = "SELECT * FROM `PetsService`.`T_Pets_Project` where project_id like '{}' ;".format(str(T_Pets_MemberProjectRole_project_id))
         
        print("in check_identifier_code: sqlStr="+sqlStr)
        resultCheck2 = check_conn.doSqlCommand(sqlStr)
        if resultCheck2["fetchall"] == [] or len(resultCheck2["fetchall"])==0:
            # response['check2'] = "no"
            print("no")
        else:
            # response['check2'] = "yes"
          
            T_Pets_Project_project_name = resultCheck2["fetchall"][0]['project_name']
            T_Pets_MemberProjectRole_project_eng = resultCheck2["fetchall"][0]['project_eng']
            T_Pets_MemberProjectRole_enc_key = resultCheck2["fetchall"][0]['enc_key']
            T_Pets_Project_issingle = resultCheck2["fetchall"][0]['issingle']
            T_Pets_Project_single_dataset = resultCheck2["fetchall"][0]['single_dataset']
            # response['T_Pets_Project_issingle'] = T_Pets_Project_issingle
            # response['T_Pets_Project_single_dataset'] = T_Pets_Project_single_dataset

##########################################
        # 是單一資料集
        if T_Pets_Project_issingle == 1: 
            group_type = T_Pets_Project_single_dataset.split('_')[0]
        else:
            sqlStr = "SELECT * FROM `PetsService`.`T_Pets_ProjectJoinFunc` where project_id like '{}' ;".format(str(T_Pets_MemberProjectRole_project_id))
             
            print("in check_identifier_code: sqlStr="+sqlStr)
            resultCheck3= check_conn.doSqlCommand(sqlStr)
            if resultCheck3["fetchall"] == [] or len(resultCheck3["fetchall"])==0:
                # response['check3'] = "no"
                print("no")
                ...
                # response['check3'] = "yes"
                print("yes")
                resultCheck3_list = resultCheck3["fetchall"]
                # response['resultCheck3_list'] = resultCheck3_list

                for i in range(len(resultCheck3_list)):
                    right_group_id = resultCheck3_list[i]["right_group_id"]
                    right_dataset = resultCheck3_list[i]["right_dataset"]
                    left_group_id = resultCheck3_list[i]["left_group_id"]
                    left_dataset = resultCheck3_list[i]["left_dataset"]
                    
                    right_dataset_name_no_csv = right_dataset.split(".csv")[0]
                    right_dataset_name_parts = right_dataset_name_no_csv.split('_')
                    right_dataset_name = '_'.join(right_dataset_name_parts[1:])
                    left_dataset_name_no_csv = left_dataset.split(".csv")[0]
                    left_dataset_name_parts = left_dataset_name_no_csv.split('_')
                    left_dataset_name = '_'.join(left_dataset_name_parts[1:])

                    # response['right_dataset_name'] = right_dataset_name
                    # response['right_dataset_name'] = right_dataset_name
                    # response['hashTableName'] = hashTableName

                    if right_dataset_name == hashTableName:
                        group_id = right_group_id
                    elif left_dataset_name == hashTableName:
                        group_id = left_group_id
                    # response['group_id'] = group_id

                    sqlStr = "SELECT * FROM `PetsService`.`T_Pets_Group` where id like '{}' ;".format(str(group_id))
             
                    print("in check_identifier_code: sqlStr="+sqlStr)
                    resultCheck4= check_conn.doSqlCommand(sqlStr) 
                    group_type = resultCheck4["fetchall"][0]['group_type']
                    # response['group_type'] = group_type
    ...
    ##########################################
        ...
        response = dict()
        errMsg = "mysql select err:"+str(e)
        response['status'] = -1
        response['errMsg'] = errMsg
        response['right_dataset_name'] = right_dataset_name
        response['left_dataset_name'] = left_dataset_name
        response['hashTableName'] = hashTableName
        # response['group_id'] = group_id
        log_time.printLog(errMsg) 
        return make_response(jsonify(response))
    ...
    #project_cert = data_raw['project_cert']
    project_cert_json = dict()
    project_cert_json["group_type"] = group_type #"h1" 
    project_cert_json["enc_key"] = T_Pets_MemberProjectRole_enc_key # "AAAAAABC0DCB39FE182FAF7CE960A2B0BA63AFEEDC76D8A92AED52938AA06ABA"  # T_Pets_MemberProjectRole_enc_key
    project_cert_json["project_name"] = T_Pets_Project_project_name #"運動"  # T_Pets_Project_project_name
    project_cert_json["project_folder"] = T_Pets_MemberProjectRole_project_eng #"test0103"  #T_Pets_MemberProjectRole_project_eng
    project_cert_json["pets_service_ip"] = service_ip # "34.81.253.109"  #
    project_cert = encodeDic(project_cert_json)

    # response['project_cert_json']=project_cert_json
    # response['project_cert']=project_cert
    # {
    #    "group_type":"h1",
    #    "enc_key":"AAAAAABC0DCB39FE182FAF7CE960A2B0BA63AFEEDC76D8A92AED52938AA06ABA",
    #    "project_name":"運動",
    #    "project_folder":"test0103",
    #    "pets_service_ip":"34.81.253.109"
    # }
    ...
    if project_cert == '' :
        response = dict()
        errMsg = 'request_error: ' + str('project_cert Empty parameter')
        errMsg = errMsg + ' ; request: {}'.format(request)
        response['status'] = -1
        response['errMsg'] = errMsg
        log_time.printLog(errMsg)
    ...
    try:
        ...
        log_time.printLog("######################")      
        log_time.printLog(Multiple_Hash_para)      
        log_time.printLog("######################")       

        response_g = requests.post("https://"+flask_ip+":"+flask_port+"/m_hash_new_async", json=Multiple_Hash_para,timeout=None, verify=False)
        response_dic = response_g.json()
...
# 20240624 ####
##############
# @app.route('/c_identifier_async', methods=['POST'])
# def c_identifier_async():
#     log_time = GetLogTime("c_identifier_async")
#     log_time.printLog("Start c_identifier_async")
...
#     identifier_code = input_["identifier_code"]
#     try:
#         task = task_check_identifier.multiplehash_longTask.apply_async((identifier_code))

#         response = dict()
#         response['projStep'] = 'check_identifier'

#         if True:
#             state_ = "test"
#             while state_ != 'PROGRESS':
#                 mac_longTask_task = task_check_identifier.multiplehash_longTask.AsyncResult(task.id)
...
@app.route("/resend_async", methods=["POST"])
@swag_from("swagger_yml/resend.yml")
def resend_async():
    log_time = GetLogTime('resend_async')
    log_time.printLog("Start resend_async")
    ...
    #get parameter
    try:
        identifier_code = data_raw['identifier']
    except Exception as e:
        response = dict()
        errMsg = 'request_error: ' + str(e)
        errMsg = errMsg + ' ; request: {}'.format(request)
    ...
    try: # fetch parameter: pid
        #sqlStr = "SELECT Progress  FROM `spark_status`.`appStatus` where proj_id like '{}';".format(proj_id,Application_Name)
        sqlStr = "SELECT * FROM `PetsService`.`T_Pets_MemberProjectRole` where key_code like '{}' ;".format(identifier_code)
        ...
        if resultCheck["fetchall"] == [] or len(resultCheck["fetchall"])==0:
            ...
            response['check'] = "yes"
            response['status'] = 1
        # response['resultChecktttt'] = resultCheck
        # response['resultCheck_test'] = resultCheck["fetchall"]
        t_fetchall = resultCheck["fetchall"]
        t_result = resultCheck["result"]
        response['resultCheck'] = {}
        response['resultCheck']["fetchall"] = t_fetchall
        response['resultCheck']["result"] = t_result
        # if int(resultCheck['result'])==1:
        #     MySQL_key_code = resultCheck["fetchall"][0]['key_code']
        check_conn.close()
    except Exception as e:
        response = dict()
        errMsg = "mysql select err:"+str(e)
    ...
    try:
        project_id = resultCheck["fetchall"][0]['project_id']
    except Exception as e:
        response = dict()
        errMsg = "keycode 已更新，請重新取得 keycode 後再上傳資料"
        response['status'] = -1
        response['errMsg'] = errMsg
        log_time.printLog(errMsg)
        return make_response(jsonify(response))
    ...
    try: # fetch parameter: pid
        #sqlStr = "SELECT Progress  FROM `spark_status`.`appStatus` where proj_id like '{}';".format(proj_id,Application_Name)
        sqlStr = "SELECT * FROM `PetsService`.`T_Pets_Project` where project_id like '{}' ;".format(project_id)
        ...
        projName = resultCheck2["fetchall"][0]['project_eng']
        response['resultCheck'] = {}
        response['resultCheck']["projName"] = projName
    ...
    pid = 999
    try:
        Multiple_Hash_para = {"projName":projName,
        ...
        response_g = requests.post("https://"+flask_ip+":"+flask_port+"/m_resend_async", json=Multiple_Hash_para,timeout=None, verify=False)
        response_dic = response_g.json()
        print("Multiple_Hash_para JSON: ",response_dic)
        # response['HashMac']='1' #response_dic['state']
        #return make_response(jsonify(response))
...
@app.route('/m_resend_async', methods=['POST'])
def m_resend_async():
    log_time = GetLogTime("m_resend_async")
    log_time.printLog("Start m_resend_async")
    ...
    try:
        task = task_resend.multiplehash_resend_longTask.apply_async((projName,projID,identifier,onlyHash))
        ...
        if True:
            state_ = "test"
            while state_ != 'PROGRESS':
                mac_longTask_task = task_resend.multiplehash_resend_longTask.AsyncResult(task.id)
                state_ = mac_longTask_task.state
                response['state'] = state_
                response['celeyId'] = mac_longTask_task.id
                #print state_
...
#20231027
@app.route("/check_identifier_async", methods=["POST"])
@swag_from("swagger_yml/check_identifier.yml")
def check_identifier_async():
    log_time = GetLogTime('check_identifier_async')
    log_time.printLog("Start check_identifier_async")
...
#20231027
@app.route('/m_hash_new_async', methods=['POST'])
def m_hash_new_async():
    log_time = GetLogTime("m_hash_new_async")
    log_time.printLog("Start m_hash_new_async")
    ...
    project_cert = input_["project_cert"]
    service_ip = input_["service_ip"]
    identifier  = input_["identifier"]
    try:
        task = task_multipleHash_new.multiplehash_longTask.apply_async((tblName,sep_,columns_mac,projName,projID,dataHash,onlyHash,AES_columns_mac,project_cert,service_ip,identifier))
        ...
        if True:
            state_ = "test"
            while state_ != 'PROGRESS':
                mac_longTask_task = task_multipleHash_new.multiplehash_longTask.AsyncResult(task.id)
                state_ = mac_longTask_task.state
                response['state'] = state_
                response['celeyId'] = mac_longTask_task.id
                #print state_
...
@swag_from("swagger_yml/MultipleHash_new.yml")
...
#20231027
# @app.route("/direct_enc_async_0110", methods=["POST"])
@swag_from("swagger_yml/MultipleHash.yml")
def direct_enc_async_0110():
    log_time = GetLogTime('direct_enc_async_0110')
    log_time.printLog("Start direct_enc_async_0110")
