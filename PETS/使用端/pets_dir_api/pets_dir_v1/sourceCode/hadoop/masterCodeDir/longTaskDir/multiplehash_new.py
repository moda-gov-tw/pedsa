...
def main__(tblName,sep,columns_mac,projName,projID,dataHash,onlyHash,AES_columns_mac,project_cert,service_ip,identifier,userId,userAccount):
    tets = 11
    ...
    _logger.debug('spark_import_service_ip_%s',service_ip)
    ...
    # if not re.match("^[a-zA-Z0-9_]+$", str(sep)):
    #     _logger.debug("Invalid sep format")
    #     return 'Fail'
    # if columns_mac != "":
    ...
    # if AES_columns_mac != "":
    #     if not re.match("^[a-zA-Z0-9_ ]+$", AES_columns_mac):
    #         _logger.debug("Invalid AES_columns_mac format")
    #         return 'Fail'

    # if not re.match("^[a-zA-Z0-9_]+$", str(project_cert)):
    #     _logger.debug("Invalid project_cert format")
    #     return 'Fail'

    # if not re.match("^[a-zA-Z0-9_ ]+$", service_ip):
    #     _logger.debug("Invalid service_ip format")
    #     return 'Fail'
    ...
    _logger.debug('project_cert = %s',project_cert)
    project_cert_json = getJsonParser(str(project_cert))
    # _logger.debug(project_cert_json)
    group_type = project_cert_json["group_type"]    
    _logger.debug('group_type = %s',group_type)
    Mac_hashkey = project_cert_json["enc_key"]    
    AES_hashkey = project_cert_json["enc_key"]
    _logger.debug('Mac_hashkey:%s',Mac_hashkey)
    _logger.debug('AES_hashkey:%s',AES_hashkey)
    ##############  0718 enc解密  ##############
    # runcode = os.system('pip install --upgrade cryptography')
    # command = "pip install --upgrade cryptography"
    # subprocess.run(command, shell=True)
    try:
        Mac_hashkey = dec(Mac_hashkey)
        AES_hashkey = dec(AES_hashkey)
    except Exception as e:
        #_logger.debug("error in QueryKeyBySchool : "+str(e))
        _logger.debug("error in dec : " + str(e))
        _logger.debug("err_dec")
    ##############  enc解密  ##############



    _logger.debug('dec AES key : %s', str(AES_hashkey))
    _logger.debug('dec Mac key : %s', str(Mac_hashkey))

   # "project_name":"運動",
   # "project_folder":"test0103",
   # "pets_service_ip":"127.0.0.1"

    # project_name = project_cert_json["project_name"]    
    project_folder = project_cert_json["project_folder"]
    projName = shlex.quote(projName)
    projName = project_folder
    _logger.debug('projName: %s',projName)
    projName = shlex.quote(projName)
    pets_service_ip = project_cert_json["pets_service_ip"]    
    # _logger.debug('project_name:%s',project_name)
    _logger.debug('project_folder: %s', str(project_folder))
    _logger.debug('pets_service_ip : %s', str(pets_service_ip))

    ##############
    project_status = 1
    statusname='直接識別 Hash 執行中'
    # statusname='AES_F'
    projID = 99999
    tblName = shlex.quote(tblName)
    tableName_ = str(group_type) + '_' + tblName # + '_enc'
    tableName_ = shlex.quote(tableName_)
    updateTProjectStatus_.updateToMysql(projID, project_status,statusname,service_ip,str(tableName_)+".csv",identifier)
    # updateToMysql_PETs(1, 99999, project_status, statusname,service_ip,str(tableName_)+".csv")
...
#---------------------------Check the AES_hashkey------------------
    ...
    if (not (onlyHash == 'N' or onlyHash == 'n' or onlyHash == "no" or onlyHash == "No" or onlyHash == "NO")):
        ...
        tableName = tblName
        projName = shlex.quote(projName)
        tableName = shlex.quote(tableName)
        data_output_project_path = "/home/hadoop/proj_/data/output/"+ projName + "/" + tableName + ".csv"
        tableName = shlex.quote(tableName)
        dataMac_input_path = "/home/hadoop/proj_/dataMac/input"+"/"+tableName+".csv"
        doCommand(['cp', data_output_project_path, dataMac_input_path])
    ...
    try:
        ...
        if not re.match("^[a-zA-Z0-9_\-/.]+$", str(file_)):
            _logger.debug("Invalid file_ format")
            return 'Fail'
        config.read(file_)
        password_ = config.get('Hadoop_information', 'password')
        tblName = shlex.quote(tblName)
        path_ = "/home/hadoop/proj_/dataMac/input/" + tblName + ".csv"
        if not re.match("^[a-zA-Z0-9_\-/.]+$", str(path_)):
            _logger.debug("Invalid path_ format")
            return 'Fail'
        cmd = 'echo '+ password_+'  | sudo -S chown hadoop:hadoop '+ shlex.quote(path_)
        ...
        _logger_fb.debug("cmd is {0}".format(str(cmd)))
        runcode = os.system(cmd)
        _logger_fb.debug("run result is {0}".format(str(runcode)))



        _logger_fb.debug("Input path: " + path_) #Input path: /home/hadoop/proj_/dataMac/output/mac_adult_id/mac_adult_id.csv
        _logger_fb.debug("----for decry----isEnc: " + str(isEnc))
    ...
    # statusname='Hash'
    # updateTProjectStatus_.updateToMysql(projID, project_status,statusname,"","")
    # updateToMysql_PETs(1, 99999, project_status, statusname,"","")
    # # updateAppStatus_.updateToMysql("Mac_data","100")
    ...
    try: 
        ...
        if 0:
            ...
            if result['result'] == 0:
                ...
                if MAC_cols == []:
                    ...
                    for column_ in MAC_cols:
                        ...
                        _logger_fb.debug("anormal count: " + str(anormal_length)+" normal count: " + \
                            str(total_length-anormal_length)+" total count: " + str(total_length))
                        #tmpDF.show(10)
                if MAC_cols == [] and AES_cols == []:
                    total_length = dfDF.count()
                    _logger_fb.debug("total_length: "+ str(total_length))
                ...
                _logger_fb.debug("out_json: " + str(out_json))
                ...
                # if not re.match("^[a-zA-Z0-9_ /]+$", str(out_dir_path)):
                #     _logger.debug("Invalid out_dir_path format")
                #     return 'Fail'

                out_json_path = out_dir_path +'/'+ tableName_ +'.json'
                # if not re.match("^[a-zA-Z0-9_\-/.]+$", str(out_json_path)):
                #     _logger.debug("Invalid out_json_path format")
                #     return 'Fail'
                # Secure the path for shell command
                safe_path = shlex.quote(out_json_path)
                with open(safe_path,'w') as json_file:
                ...
                if not re.match("^[a-zA-Z0-9_ /]+$", str(d_i_path)):
                    _logger.debug("Invalid d_i_path format")
                    return 'Fail'
                ...
                if onlyHash == 'N' or onlyHash == 'n' or onlyHash == "no" or onlyHash == "No" or onlyHash == "NO":
                    ...
                    if not re.match("^[a-zA-Z0-9_ /]+$", str(data_input_path_project)):
                        _logger.debug("Invalid data_input_path_project format")
                        return 'Fail'

                    commands =[ 'mkdir', shlex.quote(data_input_path_project)]
                    # 检查每个列表元素
                    quoted_commands = [shlex.quote(cmd) for cmd in commands]
                    doCommand(quoted_commands)

                    data_input_path_project_table = os.path.join(data_input_path_project,tableName_)
                    if not re.match("^[a-zA-Z0-9_ /]+$", str(data_input_path_project_table)):
                        _logger.debug("Invalid data_input_path_project_table format")
                        return 'Fail'
                ...
                # cmd_send = "scp -i " + key_path + " -P 22 -r " + out_dir_path + " ubuntu@"+pets_service_ip + ":/home/ubuntu/PETS/pets_service/sftp_upload_folder"
                # cmd_send = "scp -i sftp_key_123.pem" + " -r " + out_dir_path + " hadoop@"+pets_service_ip + ":/home/ubuntu/PETS/pets_service/sftp_upload_folder"
                # cmd_send = "scp -i " + key_path + " -o StrictHostKeyChecking=no -r " + out_dir_path + " sftpuser@"+pets_service_ip + ":/home/sftpuser/"

                _logger_fb.debug("============cmd_send=======================")
                _logger_fb.debug("cmd_send:%s",cmd_send)
                try: 
                    # os.system("cd /home/hadoop/proj_/longTaskDir/sftp_keys")
                    # _logger_fb.debug("cd /home/hadoop/proj_/longTaskDir/sftp_keys")
                    # os.system(cmd_send)
                ...
                # stderrLines_string = stderrLines.decode("utf-8")
                # if "Permission denied" in stderrLines_string:
                #     cmd_send = "scp -i " + key_path + " -P 22 -r " + out_dir_path + " sftpuser@"+pets_service_ip + ":/home/ubuntu/PETS/pets_service/sftp_upload_folder"
                #     _logger_fb.debug("============cmd_send=======================")
                #     _logger_fb.debug("cmd_send:%s",cmd_send)
                #     S_Command = subprocess.run(cmd_send, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                #     errLines = S_Command.stdout
...
if __name__ == "__main__":
    ...
    dataHash = sys.argv[6] 
    onlyHash = sys.argv[7]
    AES_columns_mac = sys.argv[8]
    project_cert = sys.argv[9]
    service_ip = sys.argv[10]
    identifier = sys.argv[11]
    userId = sys.argv[12]
    userAccount = sys.argv[13]
    totalLen = len(sys.argv)




    main__(tblName,sep,columns_mac,projName,projID,dataHash,onlyHash,AES_columns_mac,project_cert,service_ip,identifier,userId,userAccount)
