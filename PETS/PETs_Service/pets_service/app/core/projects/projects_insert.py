...
def insert_projects(data: DataModelIn=Body(), decode_info: dict = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        msg = f"Member {useraccount} insert Project table fail: {str(e)}"
        logger.error(msg)
        result = Result(msg=msg, status=False)
        return _result_wrapper(result, status_code=400)
    ...
    try:
        ...
        db.rollback()
        msg = f"Member {useraccount} insert join_func table fail: {str(e)}"
        logger.error(msg)
        result = Result(msg=msg, status=False)
        return _result_wrapper(result, status_code=400)
    ...
    try:
        ...
        # check_result = del_api(project_id)
        db.rollback()
        msg = f"Member {useraccount} insert project_role table fail: {str(e)}"
        logger.error(msg)
        result = Result(msg=msg, status=False)
        return _result_wrapper(result, status_code=400)
    ...
    try:
        ...
        # check_result = del_api(project_id)
        db.rollback()
        msg = f"Member {useraccount} insert project_status table fail: {str(e)}"
        logger.error(msg)
        result = Result(msg=msg, status=False)
        return _result_wrapper(result, status_code=400)
    ...
    # except Exception as e:
    #     db.rollback()
    #     msg = f"Member {useraccount} insert projects failed###: {str(e)}"
    #     logger.error(msg)
    #     result = Result(msg=msg, status=False)
    #     return _result_wrapper(result, status_code=400)
    
    msg = f"Member {useraccount} insert_projects successful"
    logger.info(msg)
