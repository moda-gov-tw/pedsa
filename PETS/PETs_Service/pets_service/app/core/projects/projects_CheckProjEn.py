...
def check_projname_En(project_eng, db: Session):

    project_eng_num = db.query(func.count()).filter(Project.project_eng == project_eng).scalar()

    if project_eng_num >= 1:
        msg = f'{project_eng} Fail, Check_Proj_En duplicated.'
        logger.info(msg)
        return False , msg

    msg = f'{project_eng} Pass, Check_Proj_En did not duplicate.'
    logger.info(msg)
    return True, msg

@Project_CheckProjEn.post("/checkprojen")
def check_proj_En(project_eng:str, decode_info: dict = Depends(verify_token), db: Session = Depends(get_db)):

    member_id = decode_info['member_id'] #從token找到who are u == 'member_id'
    useraccount = decode_info['sub']

    #Check whether project_eng is null?
    if project_eng == '': 
        msg = 'project_eng 不能為空值' 
        logger.error(msg)
        result = Result(msg=msg, status=False)
        return _result_wrapper(result, status_code=400)

    try:
        checked, msg = check_projname_En(project_eng, db)
    except Exception as e:    
        msg = f"Member {useraccount} Project_CheckProjEn failed: {str(e)}"
        logger.error(msg)
        result = Result(msg=msg, status=False)
        return _result_wrapper(result, status_code=400)
    

    # msg = f"Member {member_id} check project Project_CheckProjEn successful."
    # logger.info(msg)
