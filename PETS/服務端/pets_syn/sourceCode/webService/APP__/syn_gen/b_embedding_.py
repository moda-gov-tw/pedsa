...
#                 output_class_name = Reshape(target_shape=(int(structure[layer]/20),))(output_class_name)
                ...
                output_class_name = Embedding(structure[layer], int(5), name= self.col_name[layer]+'_embedding')(input_class_name)
                output_class_name = Reshape(target_shape=(int(5),))(output_class_name)
            elif (30>=structure[layer] > 20):
                output_class_name = Embedding(structure[layer], int(7), name= self.col_name[layer]+'_embedding')(input_class_name)
                output_class_name = Reshape(target_shape=(int(7),))(output_class_name)
            elif (20>=structure[layer] > 15):
                output_class_name = Embedding(structure[layer], int(5), name= self.col_name[layer]+'_embedding')(input_class_name)
                output_class_name = Reshape(target_shape=(int(5),))(output_class_name)  
            else:    
                output_class_name = Embedding(structure[layer], int(3), name= self.col_name[layer]+'_embedding')(input_class_name)
                output_class_name = Reshape(target_shape=(int(3),))(output_class_name)
            
            input_model.append(input_class_name)
            output_embeddings.append(output_class_name)
        
        nn = [ ]
        nn.append(int(sum(structure)*1))
        nn.append(int(sum(structure)*1.2))
...
def embedding(directory,col_name,unique_att_num):
    print("######################1")
    f = open(os.path.join(directory,'pkl','feature_train_data.pickle'), 'rb')
    (X, y) = pickle.load(f)
    ...
    if save_embeddings:
        ...
        with open(os.path.join(directory,'pkl','embeddings.pickle'), 'wb') as f:
