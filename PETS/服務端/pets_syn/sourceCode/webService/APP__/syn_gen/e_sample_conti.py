...
def sampling_conti(directory,filepath,sum_col_num,epoch=120):
    ...
    #print("epoch: ",epoch)
    original = pd.read_csv(filepath,header=0, encoding="utf-8")
...
#             df1 = df1[df1.iloc[:,sum_col_num+i]>=0.0] #'Age'
    ...
    df.to_csv(os.path.join(directory,'transform/synthetic_transform_'+str(epoch)+'.csv'),float_format='%.7f',index=False,header=True)
