...
def check_type(select_colNames,col_name,path,keyName):
    ...
    #     print("Continuous attribute:"+str(conti_col)+"\n")
        ...
        # 使用每一列的平均數替換 NaN 值
        df.loc[:, conti_col] = df.loc[:, conti_col].apply(lambda x: x.fillna(x.mean()))
        #df.loc[:, conti_col] = df.loc[:, conti_col].replace(np.nan,0.0) 
...
#     df = pd.read_csv(path,header=0)
    ...
    df_fake = pd.read_csv(os.path.join(directory,'transform','synthetic_transform_'+str(e)+'.csv' ),header=0, encoding='utf8')
    ...
    #     fake_common_all =fake_common_all.reset_index(drop=True)
        ...
        if df.shape[0] >=  fake_rm_hit.shape[0]:
            ...
            os.remove(os.path.join(directory,'transform','synthetic_transform_'+str(e)+'.csv'))
            os.remove(os.path.join(directory,'sample','synthetic_sample_'+str(e)+'.csv'))
            return "hit"
        else:
            rows = df.shape[0]
            fake_rm_hit_subset = fake_rm_hit.sample(n=rows)
            fake_rm_hit_subset.to_csv(os.path.join(directory,'synthetic','synthetic_transform_rmhit'+str(e)+'.csv'),index=False,header=True, encoding='utf-8')  
        ...
        df_fake_sample.to_csv(os.path.join(directory,'synthetic','synthetic_transform_rmhit'+str(e)+'.csv'),index=False,header=True, encoding='utf-8')  
