...
def createFolder_longTask(self, _jsonBase64,nothing):
    ...
    _logger=_getLogger('verify_createFolder_longTask')
    _vlogger=_getLogger('verify_createFolder')
    _errlogger  =_getLogger('error__createFolder')
    ...
    cmd=["python","app/devp/API/createFolder.py","-projName",projName]
    print(cmd)
...
def updateToMysql_status(self, conn,userID,projID, projName, table, return_result, errorlog):
    ...
    if resultSampleData['result'] == 1:
        ...
        errMsg = 'insert_Mysql_statusToMysql fail.'
        _errlogger.debug(errMsg)
        self.update_state(state="FAIL", meta={'Msg':errMsg,'state_no':'-2'}) 
        return  'Fail'  
def getSparkAppId(self, subprocess, viewSparkProcess_,projID,projName,userID):
    ...
    meta_={'PID': subprocess.pid,'userID':userID,'projID': projID,'projStep':'create folder','projName':projName}
    ...
    #for line in lines:
        ...
        if "error__createFolder" in  line:
            #fundTabName = 1
            kTable_index=line.find('error__createFolder - DEBUG -') 
            kTable_=line[kTable_index:]

            # tmpStrList = kTable_.split("__")
            tmpStrList = kTable_.split("error__createFolder - DEBUG -")
            #print(len(tmpStrList[1]))
            # _errlogger.debug(tmpStrList[1])
            kTable_ = tmpStrList[1][0:len(tmpStrList[1])-1]
            # print "LOG in Generation"
            print('error__createFolder: ' + tmpStrList[1])
            print ('task id is '+self.request.id)
            #print('fundTabName___________________')
            #print(fundTabName)
            #self.update_state(self.request.id, state="PROGRESS", meta={'progress': kTable_})
