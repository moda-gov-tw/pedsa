...
def PETs_preview_longTask(self, _jsonBase64,nothing):
    ...
    _logger=_getLogger('verify_PETs_preview_longTask')
    ...
    cmd=["python","app/devp/API/PETs_preview.py","-projName",projName,"-fileName",fileName,"-projID",projID,"-userID",userID]
    print(cmd)
    ...
    if Flag_complete == 'False':
        ...
        try:
            ...
            errMsg = 'errTable: updateToMysql_ProjectStatus fail.'
            _errlogger.debug(errMsg)
            return 'Fail'
...
def updateToMysql_status(self, conn,userID,projID, projName, table, return_result, errorlog):
    ...
    valueSampleData = {
            ...
            'step':'preview',
            'isRead':0,
            'return_result':list_clean(return_result),#','.join(return_result),
