...
def getMLutility_longTask(self, _jsonBase64,nothing):
    ...
    try:
        ...
        if not re.match("^[a-zA-Z0-9_ .]+$", rawDataName):
            _logger.debug("Invalid rawDataName format")
            return 'Fail'

        if not isinstance(targetCols, list):
            _logger.debug("Invalid targetCols format")
            raise 'Fail'
    ...
    cmd='python /app/app/devp/API/statisticUtility.py {} {} {} {} {} {} \"{}\"'.format(userID, projID, projName, rawDataName, rawDataDir, synDataDir, str(checkedTargetCols_))
    _vlogger.debug(cmd)
    ...
    for i in range(0,L):
        ...
        model = str(utilityResult_['model'][i])
        # MLresult = str(utilityResult_['MLresult'][i])
        MLresult_b64 = list_clean(utilityResult_['MLresult'][i])
        #print(type(MLresult))
    ...
    try:
        ...
        if stepList_[-1] == 'Mission Complete':
            updateToMysql_ProjectStatus(self, check_conn, projID, 7, u'查看報表')
        else:
...
def updateToMysql_CeleryStatus(self, conn,userID,projID, projName, table, return_result, errorlog):
    ...
    if resultSampleData['result'] == 1:
        ...
        errMsg ='insertCeleryStatusToMysql fail'
        _logger.debug(errMsg)
        self.update_state(state="FAIL", meta={'Msg':errMsg,'stateno':'-2'})
        return "Fail"
...
def updateToMysql_utilityResult(self, conn, projID, targetCol, selectCSV, model, MLresult):
    ...
    if resultSampleData['result'] == 1:
        ...
        errMsg ='insertUtilityResultToMysql fail'
        _logger.debug(errMsg)
        self.update_state(state="FAIL", meta={'Msg':errMsg,'stateno':'-2'})
        return "Fail" 
...
def getSparkAppId(self, subprocess, viewSparkProcess_, projName, projID, userID):
    ...
    while True:
        ...
        if "result - " in  line:
            #fundTabName = 1
            MLresult_index=line.find('result - ') 
            MLresult_=line[(MLresult_index+9):].strip('\n')           
        ...
        #kTable_
        if "__Statistic Utility Finished------" in  line:
            meta_['celeryId'] = self.request.id
            print('------Statistic Utility Finished------')
