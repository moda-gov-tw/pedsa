...
if __name__ == "__main__":
    ...
    old_passwd = sys.argv[2]
  
    #getConfig_ = getConfig()
        
    #test = getConfig_.getPWDFroRandomFileDB()
    #print u"{0}".format(test)
    try:
        print('start connectToMysql to check project_name in mysql: {}'.format("projName"))
        check_conn = ConnectSQL("deidadmin", old_passwd)
    ...
    try:
        dbName="mysql"
        tblName="user"
        #values = "authentication_string=password("
        values = "Password=password("
        values = values+"'"+pwdd+"'"
        print(values)
        print("----------")

        conditions = "User="+"'"+"deidadmin"+"'"+";"

        sqlStr = "UPDATE {}.{} ".format(dbName, tblName)
        #sqlStr = sqlStr + "SET {},updatetime = now()".format(values)
        sqlStr = sqlStr + "SET {})".format(values)
        sqlStr = sqlStr + " WHERE {}".format(conditions)
        #sqlStr = sqlStr+"flush privileges;"
        #print(sqlStr.encode('utf8'))
        ret = check_conn.doSqlCommand(sqlStr.encode('utf8'))
        ...
        print('doSqlCommand fail: ' + str(e))
        sys.exit(str(e))

#UPDATE mysql.user1 SET authentication_string=password('citcw200')) WHERE user='keyadmin' and Host ='localhost';
#update mysql.user set authentication_string=password('新密碼') where user='root' and Host ='localhost';
#UPDATE mysql.user SET authentication_string=password('citcw200) WHERE User='keyadmin' and Host ='localhost';
