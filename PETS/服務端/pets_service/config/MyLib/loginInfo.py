...
class getConfig():
	...
	def __init__(self):
		...
		self.config.read('/usr/src/app/config/development.ini')
	...
	'''
	def getLoginMysql(self):
		ip = self.config.get('webservice', 'ip')
		port = self.config.get('webservice', 'port')
		user = self.config.get('webservice', 'user')
		password = self.config.get('webservice', 'password')
		sql_type = self.config.get('webservice', 'sql_type')
