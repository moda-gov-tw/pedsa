...
def db_query_syslog(starttime, endtime, db: Session):
    db_syslog_list = db.query(Syslog).filter(Syslog.sysdatetime <= endtime).filter(
        Syslog.sysdatetime >= starttime).order_by(Syslog.sysdatetime.desc()).order_by(Syslog.id.desc())
    list_syslog_ser = ListSyslog.model_validate(db_syslog_list).model_dump()
    return list_syslog_ser

def db_check_member_reference_table_data_exist(delete_member_id, db: Session):
    reference_table = []
    metadata = db.bind
    
    model_list = [Permission, RolePermission, Member, MemberGroupRole, FailedLogin,MemberProjectRole, Group, ProjectStatus, Syslog]
    ...
    for table_name in table_names:
        ...
        for foreign_key in foreign_keys:
            ...
            if referred_table == Member.__tablename__ and referred_columns == ['id']:
                for model in model_list:
                    model_table_name = class_mapper(model).class_.__tablename__
                    if model_table_name == table_name:
                        reference_table.append({
                            'class': class_mapper(model).class_,
                            'column_name': foreign_key['constrained_columns'][0]
                        })
    
    has_delete_member_id = []     
    for ref_table in reference_table:
        column_name = ref_table['column_name']
        query_result = db.query(ref_table['class']).filter(getattr(ref_table['class'], column_name) == delete_member_id).all()
        if query_result:
            has_delete_member_id.append(
                {
                    'class':ref_table['class'] ,
                    'table_name' : ref_table['class'].__tablename__
                })

    return has_delete_member_id

def user_is_active(member_id, db: Session):
    member = db.query(Member).filter(Member.id == member_id).first()
    if not member:
        return False, 'no member exist'
    if not member.isactive:
...
def compare(input,table):
    ...
    not_match = []
    for i in range(len(input.permissions)):
        if input.permissions[i] != table['permissions'][i]:
            not_match.append(error_dict[i])
    if input.owner != table['owner']:
            not_match.append(error_dict[10])
    if input.group != table['group']:
            not_match.append(error_dict[11])
    if not_match != []:
        output  = input.folder_name + ':' + ','.join(not_match)
        return output
    else: 
        return 'match'


def get_permission_string(st_mode):
    is_dir = 'd' if st_mode & S_IFDIR else '-'    
    dic = { S_IRUSR: 'r', S_IWUSR: 'w', S_IXUSR: 'x',
        S_IRGRP: 'r', S_IWGRP: 'w', S_IXGRP: 'x',
        S_IROTH: 'r', S_IWOTH: 'w', S_IXOTH: 'x',
    }    
    perm = [dic.get(st_mode & mask, '-') for mask in [
    S_IRUSR, S_IWUSR, S_IXUSR,S_IRGRP, S_IWGRP, S_IXGRP,S_IROTH, S_IWOTH, S_IXOTH]]    
    return is_dir + ''.join(perm)

def file_info(entry,folder_name=''):
    if not isinstance(entry,os.stat_result):
        info_ = entry.stat()
        result = subprocess.run(['ls', '-l', os.path.join(folder_name,entry)], capture_output=True, text=True, check=True)
        ...
        if len(line) >=2:
            ...
            group = line[3]
            # 獲取擁有者和群組名稱
            if entry.is_dir() and len(line)>=5:
                owner = line[4]
                group = line[5]

            permissions = get_permission_string(info_.st_mode)
        ...
        result = subprocess.run(['ls', '-l', folder_name], capture_output=True, text=True, check=True)
        ...
        if len(line) >=2:
            ...
            group = line[3]
            # 獲取擁有者和群組名稱
            if os.path.isdir(folder_name) and len(line)>=5:
                owner = line[4]
                group = line[5]
        
        permissions = get_permission_string(entry.st_mode)
        return {
