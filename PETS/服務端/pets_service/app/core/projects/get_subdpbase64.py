...
def get_info_fromdb(dp_pid):
    sqlStr = f"SELECT pro_name, file_name, pro_col_en FROM DpService.T_ProjectColumnType WHERE project_id = '{dp_pid}'"
    try:
        ...
        if resultSampleData['result'] == 1:
            ...
            pro_col_en=resultSampleData['fetchall'][0].get('pro_col_en')
            return pro_name, file_name, pro_col_en
        else:
...
def get_select_colNames(dp_pid):
    sqlStr = f"SELECT select_colNames FROM DpService.T_ProjectSample5Data WHERE project_id = '{dp_pid}'"
    try:
        check_conn = ConnectSQL()
        resultSampleData = check_conn.doSqlCommand(sqlStr)
        if resultSampleData['result'] == 1:
...
def get_colValues(dp_pid):
    sqlStr = f"SELECT selectcolvalue FROM DpService.T_ProjectColumnType WHERE project_id = '{dp_pid}'"
    try:
        check_conn = ConnectSQL()
        resultSampleData = check_conn.doSqlCommand(sqlStr)
        if resultSampleData['result'] == 1:
            selectcolvalue = resultSampleData['fetchall'][0].get('selectcolvalue')
            return selectcolvalue
        else:
            print('fetch DataToMysql fail')
            check_conn.close()
            return None
...
def dp_base64(dp_pid):
    curl_compose= dict()
    curl_compose['userID'] = str(1)
    curl_compose['projID'] = str(dp_pid)
    pro_name, file_name, cols  =  get_info_fromdb(dp_pid)
    curl_compose['projName'] = pro_name
    curl_compose['fileName'] = file_name
    curl_compose['colNames'] = cols.split(',')
    curl_compose['select_colNames'] =  get_select_colNames(dp_pid).split(',')
    curl_compose['select_colValues'] =  get_colValues(dp_pid).split(',')
