...
Project_SingleInsert = APIRouter() ##need to edit
...
@Project_SingleInsert.post("/singleinsert") ##need to edit
def singleinsert_projects(data: DataModelIn=Body(), decode_info: dict = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    logger.info(f"***********insert_singleprojects*************")
    checked=True
    ...
    msg = f"Member  insert_single_project_information successful"
    logger.info(msg)
