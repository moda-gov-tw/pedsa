...
Project_CheckGpProjnum = APIRouter()
...
def Check_Gp_Proj_Num(member_id, db: Session):
    gp_num_dict = dict()

    user_group_id = db.query(Member).filter(Member.id == member_id).first().group_id #userID反查user所屬gp_id
    gp_project_quota_limit = db.query(Group).filter(Group.id == user_group_id).first().project_quota #gp_id反查gp的專案上限
    gp_project_num = db.query(func.count()).filter(Project.group_id == user_group_id).scalar()

    gp_num_dict['gp_project_quota_limit'] = gp_project_quota_limit
    gp_num_dict['gp_project_num']= gp_project_num

    return True, gp_num_dict

@Project_CheckGpProjnum.get("/checkgpprojnum")
def check_gp_proj_num(decode_info: dict = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    try:
        checked, gp_num_dict = Check_Gp_Proj_Num(member_id, db)
    except Exception as e:    
        msg = f"Member {useraccount} Project_CheckGpProjnum failed: {str(e)}"
        logger.error(msg)
        result = Result(msg=msg, status=False)
        return _result_wrapper(result, status_code=400)
    

    msg = f"Member {member_id} check project CheckGpProjnum successful."
    logger.info(msg)
    result = Result(msg=msg, MemberID=member_id, obj=gp_num_dict, status=checked)
