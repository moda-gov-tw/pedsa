    ...
    try:
        for exec_cmd in delete_dirs:
            ...
            cmd = [
                ...
                "-P", "22",
                f"{user}@{ip}",
                "sudo", "chown", "-R", "ubuntu:ubuntu" ,exec_cmd
            ]
            cmd = shlex.quote(cmd)
...
def delete_projects(project_id: int = Form(), decode_info: dict = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        msg = f"Member delete project failed: {str(e)}"
        logger.error(msg)
        result = Result(msg=msg, status=False)
        return _result_wrapper(result, status_code=400)
    ...
    msg = f"Member delete_projects  successful (History)"
    logger.info(msg)
    ...
    try:
        ...
        db.rollback()
        msg = f"Member  delete sub-system project failed: {str(e)}"
        logger.error(msg)
        result = Result(msg=msg, status=False)
        return _result_wrapper(result, status_code=400)
