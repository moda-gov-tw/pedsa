    ...
    if not is_super_admin: #Groupadmin or user
        ...
        if not is_group_admin:
            ...
            #     user_dict = user.model_dump()
                ...
                proj_dict['issingle'] =db.query(Project).filter(Project.project_id == proj.project_id).first().issingle
        ...
        project_ls = []
        #logger.info(f"***********11111111111111111111111**************")
        #logger.info(project_ls)
        #logger.info(f"***********22222222222222222222222222**************")
        for proj in all_project  :
        #     user_dict = user.model_dump()
            proj_dict = dict()
            proj_dict['project_id'] = proj.project_id
            ...
            proj_dict['project_role'] = 3 #不知道要從哪邊撈，先設3
            proj_dict['issingle'] =proj.issingle
            if proj_status.updatetime is None:
                proj_dict['project_time'] =proj_status.createtime
            else:
                proj_dict['project_time'] =proj_status.updatetime
...
def list_projects(decode_info: dict = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    try:
        ...
        msg = f"Member  project list failed: {str(e)}"
        logger.error(msg)
        result = Result(msg=msg, status=False)
        return _result_wrapper(result, status_code=400)
    ...
    # if not checked:
    #     msg = f"List projects failed: Member   does not have project_list permission"
    #     logger.error(msg)
    #     result = Result(msg=msg, status=False)
    #     return _result_wrapper(result, status_code=400)

    msg = f"Member   list projects successful"
    logger.info(msg)
