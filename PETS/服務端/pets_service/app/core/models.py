...
class MemberProjectRole(Base):
    ...
    createmember_id = Column(Integer, ForeignKey(Member.id), nullable=False, comment="建立使用者ID")
    updatemember_id = Column(Integer, ForeignKey(Member.id), nullable=True, comment="修改使用者ID")
    key_code = Column(VARCHAR(100), nullable=True, comment="憑證代碼")
...
class Project(Base):
    ...
    enc_key = Column( LONGTEXT, nullable=False)
    ...
    issingle = Column(Integer, nullable=True)
    single_dataset = Column( VARCHAR(100), nullable=False)
...
class HistoryProject(Base):
    ...
    enc_key = Column( LONGTEXT, nullable=False) #Project:T_Pets_Project
...
class UtilityResult(Base):
    ...
    createMember_Id = Column(Integer, nullable=True)
    updateMember_Id = Column(Integer, nullable=True)
    utilitylevel = Column( VARCHAR(255), nullable=False)
...
class FilePermission(Base):
    __tablename__ = "T_Pets_folder_Permission"
    # row_num = Column(Integer,primary_key=True, nullable=False)
    folder_name = Column(VARCHAR(255),primary_key=True, nullable=False)
    ...
    is_directory = Column(Integer, nullable=False)
