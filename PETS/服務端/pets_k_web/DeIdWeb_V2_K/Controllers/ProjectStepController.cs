            ...
            try
            {
                ...
                if (lst.Count > 0)
                {
                    foreach (var item in lst)
                    {
                        ...
                        var warning_html = "<tr><td class='half_col'>{0}</td><td class='half_col text_right'>{1}<img src='/images/warning.png' title='隱私洩漏風險，針對非直接與間接之欄位也就是將敏感與不處理欄位進行隱私洩漏檢查，低於風險值(K值)的資料筆數數量加總顯示，提供使用者進行隱私洩漏風險判定'></td></tr>";
                        var warning_nodata_html = "<tr><td class='half_col'>{0}</td><td class='half_col text_right'>{1}</td></tr>";
                        ...
                        {
                            warning_html_table += String.Format(warning_nodata_html, "無資料", "");
                            warning_html_table += ",";
                        }
                    }
