...
if [ "$1" == "dev" ]; then
    ...
    sed -i "s/DOCKER_WEB_OUTER_PORT=.*/DOCKER_WEB_OUTER_PORT=\'$NEXTAUTH_PORT\'/g" $baseDir/.env
    sed -i "s/DOCKER_WEB_INNER_PORT=.*/DOCKER_WEB_INNER_PORT=\'$NEXTAUTH_PORT\'/g" $baseDir/.env
