...
const handleProjectJoin = async (session, projectDetail, setJoinStatus, setErrorMsg) => {
    ...
    if(projectDetail.issingle == 1){
        newArray.push({
            "dataset": projectDetail.single_dataset
        })
    }else{
    ...
    const payload = {
        ...
        join_type: projectDetail.issingle == 1 ? 2 : projectDetail.join_type,
        join_func: newArray,
    };
    ...
    promise.then((response) => {
        ...
        const status = response.data.status;
        if (status != 0) {
            console.log("資料鏈結時發生錯誤 /projects/joindata -> response.data.msg", response.data.msg);
            setErrorMsg(prev => ({ ...prev, ["join"]: "要求執行資料鏈結時發生錯誤" }));
        }
...
const DataConnectProcess = () => {
     ...
     ********/
    return (
        <Page title="data connect process">
            {/* 頂部進度條 */}
            ...
            <Box sx={{ width: '100%', mb: "20px", mt: "50px" }}>
                <Stack direction="column" spacing={1} sx={{ minWidth: '90%' }}>
                    <Typography variant='h3' sx={{ marginBottom: "20px" }}>資料鏈結處理</Typography>
                    <Divider />
                </Stack>
            </Box >
