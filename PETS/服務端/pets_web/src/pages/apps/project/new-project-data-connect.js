...
const NewProjectDataConnect = () => {
  ...
  const handleMultiSave = async () => {
    // Fetch API /projects/multiinsert
    ...
    if (checker(goSave)) {
      ...
      let projectInsertPayload = {
        ...
        'join_type': join_method_dic[dataJoinMethod],
        'join_func': dataConnectSettings,
        "aes_col": "",
        "jointablecount": 0,
        "jointablename": "",
      }
      const url = "/api/project/post_multiinsert";
      const payload = projectInsertPayload; // [TODO] need to fill in dynamic data
      // console.log('payload', payload);
      const config = {
        headers: {
  ...
  const renderColumnSettingContent = ({ columnSettingCount, dataConnectSettings }) => {
    ...
    for (let i = 1; i < columnSettingCount; i++) {
      ...
      content.push(
        <Grid container sx={{ margin: "20px 0 0 35px" }}>
          <Grid item xs={2} />
          <Grid item xs={9}>
            <Stack direction='row'>
  ...
  return (
    <Page title="new project data connect">
      ...
      <Box sx={{ width: {xs:'90%',xl:'100%'}, mb: "20px", mt: "50px" }}>
        <Grid container spacing={6} sx={{ ml: "50px", maxWidth: "1200px", }}>
          ...
          <Grid container sx={{ margin: "20px 0 0 35px" }}>
            <Grid item xs={2} spacing={2}>
              <InputLabel sx={{ mb: "20px" }}>*資料鏈結欄位屬性設定</InputLabel>
            </Grid>
            <Grid item xs={9}>
              <List sx={{ marginBottom: "20px", marginLeft: "10px" }}>
        ...
        <Grid container row sx={{ marginTop: "20px", marginLeft: "15px" }}>
          <Grid container spacing={6} columns={15}>
            ...
            <Grid item xs={4} xl={3} sx={{ display: "flex", flexDirection: "column", alignItems: "flex-end", justifyContent: "flex-end" }}>
              <Button variant="outlined" sx={{ marginLeft: "10px", marginTop: "30px" }} onClick={() => { setColumnSettingCount(columnSettingCount + 1) }} startIcon={<AddIcon />}>新增鏈結組合
              </Button>
              <Button
                variant="contained"
        ...
        <Grid container spacing={6}>
          ...
          <Button variant="outlined" sx={{ marginLeft: "10px", marginTop: "30px" }} onClick={() => { setColumnSettingCount(columnSettingCount + 1) }} startIcon={<AddIcon />}>新增鏈結組合
            ...
            <Button
              ...
              fullWidth
              onClick={handleSave}
              sx={{ marginLeft: "10px", marginTop: "20px" }}>儲存設定
            </Button>
          </Grid>
        </Grid>
      </Grid> */}
