...
const handleFetchSubSystemStatus = async (session, projectDetail, url) => {
  ...
  if (promiseResult && promiseResult?.status == 200) return promiseResult?.data[0];
  else return promiseResult;
};
...
const requestSystemInfo2 = async (session, projectDetail, innerCheckStatusURL, innerConnURL) => {
  ...
  try {
    ...
    } else throw { status: 500, message: '[Custom] Unexpected error occurred', response: checkStatusPromiseResult };
  } catch (error) {
    console.log('[Error]\n', error);
...
const PrivacyEnhancement = () => {
  ...
  useEffect(() => {
    ...
    if (projectDetail) {
      ...
      // K
      const promiseK = requestSystemInfo2(session, projectDetail, '/api/project/get_k_checkstatus', '/api/project/get_k_conn').then(
        (response) => {
      ...
      // GAN
      const promiseGan = requestSystemInfo2(session, projectDetail, '/api/project/get_syn_checkstatus', '/api/project/get_syn_conn').then(
        (response) => {
      ...
      // DP
      const promiseDP = requestSystemInfo2(session, projectDetail, '/api/project/get_dp_checkstatus', '/api/project/get_dp_conn').then(
        (response) => {
      ...
      // join togather
      Promise.all([promiseK, promiseGan, promiseDP]).then((res) => {
        setWaittingCheckSubSystem(false);
      });
  ...
  }, [clockCount, gotQuery, projectDetail, userPermission]);

  useEffect(() => {
    setTimeout(() => {
      if (!(gotStatusK & gotStatusGan & gotStatusDP)) {
        let stringBadSystems = '';
        if (!gotStatusK) stringBadSystems += 'K匿名, ';
        if (!gotStatusGan) stringBadSystems += '合成, ';
        if (!gotStatusDP) stringBadSystems += '差分隱私, ';
        stringBadSystems = stringBadSystems.slice(0, -2);

        setErrorMsg((prev) => ({
          ...prev,
          ['subsystemNotReady']: '錯誤: ' + stringBadSystems + '資料未就緒或專案遺失，請返回專案列表頁面點選「重設專案」'
        }));
      }
    }, 3000);
  ...
  useEffect(() => {
    ...
    const systemID2StatusList = {
      KAnonymous: ['可進行概化規則設定', '隱私強化處理中', '查看報表與資料匯出', '資料匯出中', '可下載隱私強化資料'],
      syntheticData: ['可進行概化規則設定', '資料概化與合成中', '可進行資料相似度比對', '資料相似度比對中', '查看報表與資料匯出', '資料匯出中', '可下載隱私強化資料'],
      differentialPrivacy: ['可進行概化規則設定', '資料概化與生成中','可進行關聯欄位屬性設定', '可進行隱私強化處理', '隱私強化處理中', '可進行資料相似度比對', '資料相似度比對中', '可下載隱私強化資料']
    };
    ...
    const [backendFinalStatusK, backendFinalStatusGan, backendFinalStatusDP] = [11, 10, 10];
    const [frontendStepperLengthK, frontendStepperLengthGan, frontendStepperLengthDP] = [4, 5, 5];
    // K
    ...
    const mappingStatusNameTableGan = [
      ...
      { range: [3, 3], newState: 0 }, // 顯示文字: 可進行概化規則設定
      { range: [4, 4], newState: 1 }, // 顯示文字: 資料概化與合成中
      { range: [5, 5], newState: 2 }, // 顯示文字: 可進行資料相似度比對
      { range: [6, 6], newState: 3 }, // 顯示文字: 資料相似度比對中
      { range: [7, 7], newState: 4 }, // 顯示文字: 查看報表與資料匯出
      { range: [8, 9], newState: 5 }, // 顯示文字: 資料匯出中
      { range: [backendFinalStatusGan, backendFinalStatusGan], newState: 6 } // 顯示文字: 可下載強化資料
    ];
    // sub-project status map to stepper circle status
    const mappingStatusStepperTableGan = [
      { range: [-1, readyStatusGan], newState: -1 }, // (step 0)
      // { range: [, ], newState: 0 }, // (step 1)
      { range: [3, 4], newState: 1 }, // (step 2)
      { range: [5, 7], newState: 2 }, // (step 3)
      { range: [8, 9], newState: 3 }, // (step 4)
      { range: [backendFinalStatusGan, backendFinalStatusGan], newState: 4 } // (step 5)
    ];
    // DP
    // sub-project status map to stepper label name
    // differentialPrivacy: ['可進行關聯屬性設定', '可進行隱私強化處理', '隱私強化處理中','可進行資料相似度比對', '資料相似度比對中', '可下載強化資料']

    const mappingStatusNameTableDP = [
      { range: [0, readyStatusDP], newState: -1 }, // 顯示文字: 資料已就緒
      // { range: [3, 3], newState: ? }, // 顯示文字: 欄位屬性設定中...
      { range: [3, 3], newState: 0 }, // 顯示文字: 可進行關聯屬性設定
      { range: [4, 4], newState: 1 },
      { range: [5, 5], newState: 2 }, // 顯示文字: 可進行隱私強化處理
      { range: [6, 6], newState: 3 }, // 顯示文字: 可進行資料相似度比對
      { range: [7, 7], newState: 4 }, // 顯示文字: 隱私強化處理中
      { range: [8, 8], newState: 5 },
      { range: [9, 9], newState: 6 },
      { range: [backendFinalStatusDP, backendFinalStatusDP], newState: 7 } // 顯示文字: 可下載強化資料
    ];
    // sub-project status map to stepper circle status
    const mappingStatusStepperTableDP = [
      { range: [-1, readyStatusDP], newState: -1 }, // (step 0) 匯入 ~ 就緒
      // { range: [3, 3], newState: ? }, // (step ?) 欄位屬性設定中...
      { range: [3, 4], newState: 0 }, // (step 1) 可進行關聯屬性設定
      { range: [5, 6], newState: 2 }, // (step 2) 可進行隱私強化處理
      { range: [7, 8], newState: 3 }, // (step 3) 隱私強化處理中
      { range: [9, 9], newState: 4 }, // (step 3) 隱私強化處理中
      { range: [backendFinalStatusDP, backendFinalStatusDP], newState: 5 } // (step 4) 可下載強化資料
    ];
    ...
    function mapStateToNewLabelState(status, mappingTable, systemName) {
      ...
      if (systemName == 'KAnonymous') {
        if (status == 93) return -7;
        else if (status == 94) return -8;
        else if (status == 95) return -9;
    ...
    function mapStateToNewStepperState(status, mappingTable, systemName) {
      ...
      if (systemName == 'KAnonymous') {
        if (status == 93) return 0; // step 1
        else if (status == 94) return 2; // step 3
        else if (status == 95) return 2; // step 3
      } else if (systemName == 'syntheticData') {
        if (status == 98)
          // 資料合成錯誤: 顯示 step 2
          return 1;
    ...
    if (gotStatusK & gotStatusGan & gotStatusDP) {
      ...
      var rawData = {
        KAnonymous: {
          ...
          url:
            subsystemInfos_K.return_url == '' || subsystemInfos_K.return_url == undefined || subsystemInfos_K.return_url == null
              ? ''
              : `${subsystemInfos_K.return_url}&loginname=${loginname}&returnurl=${returnurl}`,
          links: {
        ...
        syntheticData: {
          ...
          frontendStepperStatus: mapStateToNewStepperState(
            subsystemInfos_GAN.project_status,
            mappingStatusStepperTableGan,
            'syntheticData'
          ), // frontend stepper current status
          ...
          url:
            subsystemInfos_GAN.return_url == '' || subsystemInfos_GAN.return_url == undefined || subsystemInfos_GAN.return_url == null
              ? ''
              : `${subsystemInfos_GAN.return_url}&loginname=${loginname}&returnurl=${returnurl}`,
        ...
        differentialPrivacy: {
          ...
          frontendStepperStatus: mapStateToNewStepperState(
            subsystemInfos_DP.project_status,
            mappingStatusStepperTableDP,
            'differentialPrivacy'
          ), // frontend stepper current status
          ...
          url:
            subsystemInfos_DP.return_url == '' || subsystemInfos_DP.return_url == undefined || subsystemInfos_DP.return_url == null
              ? ''
              : `${subsystemInfos_DP.return_url}&loginname=${loginname}&returnurl=${returnurl}`,
          links: {
      ...
      const preprocessedAPI = (api_obj) => {
        ...
        for (const key_systemID in api_obj) {
          ...
          if (!oneCanDownload) {
            if (isFinalStatus) {
              ...
              } else if (projectStatus > 5) setOneCanDownload(true);
            }
          }
          ...
          if (key_systemID == 'differentialPrivacy') {
            ...
            linksArray.push({
              ...
              disabled: canDownloadProject && isFinalStatus ? false : true
            });
          } else {
            const routerConfigDownloadButton = {
              pathname: '/apps/project/data-download',
          ...
          if (system.links.decrypt) {
            linksArray.push({
              ...
              disabled: canOperateProject && projectDetail.aes_col == '' ? true : isFinalStatus ? false : true
            });
          }
            ...
            linksArray.push({
              ...
              clickFunc: () => {
                setResetInfo({
                  ...
                  agreeButtonText: '確定',
                  agreeButtonOnClick: (event) =>
                    handleReset(
                      session,
                      key_systemID,
                      projectDetail,
                      system.subsystemProjectID,
                      system.links.reset,
                      setOpenResetDialog,
                      setResetInfo
                    )
              ...
              disabled: canResetProject && system.backendCurrentStatus == system.backendReadyStatus ? true : false
            });
        ...
        // No one can download
        if (
          isFinalStausObject.KAnonymous == false &&
          isFinalStausObject.syntheticData == false &&
          isFinalStausObject.differentialPrivacy == false
        ) {
  ...
  const handleClickBlockButton = (event, url) => {
    if (-1 < projectStatus && projectStatus <= 4) handleSetProjectStatus(session, project_id, 5);
  ...
  const handleProjectView = () => {
    router.push({
      ...
      query: {
        project_id: project_id,
        project_name: project_name,
        return_url: '/apps/project/privacy-enhancement'
        // project_group_id: prop.project_group_id,
      }
    });
  };
   ...
   ********/
  return (
    <Page title="privacy enhancement">
      {/* 頂部進度條 */}
      <Box sx={{ width: '750px', margin: '10px auto 60px auto' }}>
              ...
              {loading ? (
                ...
                <>
                  {Object.keys(preprocessedData).map((key_systemID, index) => {
                    ...
                    return (
                      <Stack direction="column" spacing={1} sx={{ minWidth: '30%', padding: '40px 20px 0 20px' }}>
                        {/* Block Button */}
                        {system.url == '' || !canOperateProject ? (
                          <StatusButton
                            currentStatus={-1}
                            triggrtStatus={Number(system.backendReadyStatus)}
                            buttonName={systemID2Name[key_systemID]}
                            minHeight={'38%'}
                          />
                        ) : (
                          <StatusButton
                            currentStatus={Number(system.backendCurrentStatus)}
                            triggrtStatus={Number(system.backendReadyStatus)}
                            buttonName={systemID2Name[key_systemID]}
                            minHeight={'38%'}
                            onClickFunc={(e) => {
                              handleClickBlockButton(e, system.url);
                            }}
                          />
                        )}
                        ...
                        {/* Stepper */}
                        <GeneralStepper
                          currentStep={Number(system.frontendStepperStatus)}
                          terminatedStep={Number(system.frontendStepperLength)}
                          StepName={systemID2StatusLabelObject[key_systemID][system.frontendLabelStatus]}
                        />
      ...
      {openResetDialog ? (
        <StateControlDialog
          stateArrayOpenControl={[openResetDialog, setOpenResetDialog]}
          dialogTitle={resetInfo.dialogTitle}
          dialogContent={resetInfo.dialogContent}
          disagreeButtonText={resetInfo.disagreeButtonText}
          agreeButtonText={resetInfo.agreeButtonText}
      ...
      <Box display="flex" justifyContent="space-between" sx={{ width: '90%', margin: '0 60px 100px 60px' }}>
        <Button variant="contained" onClick={handleProjectView} sx={{ width: '150px' }}>
          檢視專案
        </Button>
        <Button
          variant="contained"
