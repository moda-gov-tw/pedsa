  ...
  const doPasswordReset = async () => {
    await axios
      ...
      .then(async (res) => {
        ...
        await setPopUpMsg(
          <div>
            修改密碼成功，預設密碼為 {res.data.obj.new_password}
            <Tooltip title="複製密碼" placement="top">
              <IconButton
                onClick={(event) => {
                  handleCopyClick(event, res.data.obj.new_password);
                }}
              >
  ...
  return (
    <Stack direction="row" alignItems="center" justifyContent="center" spacing={0}>
      ...
      >
        ...
        {popUpMsg && isPasswordResetSuccess ? (
          <DialogActions>
            <Button
              variant="contained"
              onClick={() => {
...
const UserListTable = () => {
  ...
  const getUserInfo = async (token) => {
    ...
    await axios
      .get(`/api/user/get_info/${loginUser.id}`, {
        headers: {
          ...
          <>
            <MainCard content={false}>
              <ScrollX>
                <ReactTable
                  columns={columns}
                  data={allUsers.filter((u) => u.group_id === userInfo.group_id)}
                  handleOpenAddDialog={handleOpenAddDialog}
                />
              </ScrollX>
            </MainCard>
            {/*新增/移除人員為專案管理員*/}
            <AssignUserProjectAdmin
              user={user}
              userId={userId}
              userIsProjectAdmin={userIsProjectAdmin}
              userProjectAdminId={userProjectAdminId}
              allUsers={allUsers}
              setAllUsers={setAllUsers}
              open={openAssignProjectAdmin}
              handleClose={handleAssignProjectAdminDialog}
            />
          </>
        ) : (
          <>
            ...
            <AlertUserActivate
              ...
              handleClose={handleActivateDialog}
            />
          </>
        ))}
    </Page>
