...
const AuthLogin = ({ providers, csrfToken }) => {
  ...
  const checkSystemStatus = async () => {
    try {
      const [containerStatus, filePermissionStatus, dbStatus] = await Promise.all([
        axios.get('/api/sys/get_containersStatus'),
        axios.get('/api/sys/get_filePermission'),
        axios.get('/api/sys/get_checkDb'),
      ]);

      return containerStatus.data.is_connection && filePermissionStatus.data.is_match && dbStatus.data.is_ready;
    } catch (error) {
    ...
    <>
      <Formik
        ...
        onSubmit={async (values, { setErrors, setSubmitting }) => {
          ...
          const systemCheckPassed = await checkSystemStatus();

          if (!systemCheckPassed) {
            setSystemError('系統狀態檢查失敗，請先確保系統容器、資料夾權限及資料庫有效。');
            setSubmitting(false);
            return;
          }
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                <Stack spacing={1}>
                  ...
                  {systemError && (
                    <Grid item xs={12}>
                      <FormHelperText error>{systemError}</FormHelperText>
                    </Grid>
                  )}
                </Stack>
              </Grid>
