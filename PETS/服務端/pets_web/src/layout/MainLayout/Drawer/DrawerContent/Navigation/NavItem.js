...
const NavItem = ({ item, level }) => {
  // console.log('item.title in NavItem', item.title);
      ...
      {menuOrientation === LAYOUT_CONST.VERTICAL_LAYOUT || downLG ? (
        ...
        >
          ...
          {(drawerOpen || (!drawerOpen && level !== 1)) && (
            <ListItemText
              primary={
                item.title.props.id.split('/').length > 1 ? (
                    <Typography variant="h6" sx={{ color: isSelected ? iconSelectedColor : textColor }}>{item.title.props.id.split('/')[item.title.props.id.split('/').length-1]}</Typography>
                ) : (
                    <Typography variant="h6" sx={{ color: isSelected ? iconSelectedColor : textColor }}>{item.title}</Typography>
                )
          ...
          <div style={{ display: 'none' }}>listitem</div>
        </ListItemButton>
      ) : (
        <ListItemButton
          {...listItemProps}
