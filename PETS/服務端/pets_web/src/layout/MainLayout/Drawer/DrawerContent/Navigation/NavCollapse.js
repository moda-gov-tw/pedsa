...
const NavCollapse = ({ menu, level, parentId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }) => {
  ...
  const { userPermission, setUserPermission } = useContext(ConfigContext);
  ...
  const navCollapse = menu.children?.map((item) => {
    const renderNavItem = () => {
      if (menu.id === 'systemInfo' && userPermission.includes('project_admin') && !userPermission.includes('super_admin')) {
        if (item.id !== "sys-health-test" && item.id !== "sys-log-record") {
