...
namespace DeIdWeb.Infrastructure.Service
{
    public class MySqlDBHelper
    {
        ...
        public void CancelProjectDeleteData(int ps_id)
        {
            ...
            try
            {
                ...
                string delstr = "Delete from T_GANStatus where project_id=@project_id;" +
                    "Update T_ProjectColumnType set ob_col='',corr_col='',choose_corr_col='',updatetime=now() where project_id=@project_id ;" +
        ...
        public bool UpdateProjectftaskid(int ps_id, int ftaskid)
        {
            Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
           
            string Connection = Configuration["ConnectionStrings:DefaultConnection"];
            //string Connection = Configuration["ConnectionStrings:DpConnection"];
            try
            {
                MySqlConnection conn = new MySqlConnection(Connection);
                conn.Execute("update T_Project set ftaskid = @ftaskid where project_id = @project_id", new { project_id = ps_id, ftaskid = ftaskid });
                return true;
            }
            catch (Exception ex)
            {
                log.Error("UpdateProjectftaskid Exception :" + ex.Message);
                return false;
            }
        }


        public bool UpdateProjectsectaskid(int ps_id, int sectaskid)
        {
            Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
            string Connection = Configuration["ConnectionStrings:DefaultConnection"];
            try
            {
                MySqlConnection conn = new MySqlConnection(Connection);
                conn.Execute("update T_Project set sectaskid = @sectaskid where project_id = @project_id", new { project_id = ps_id, sectaskid = sectaskid });
                return true;
            }
            catch (Exception ex)
            {
                log.Error("UpdateProjectsectaskid Exception :" + ex.Message);
                return false;
            }
        }

        public bool UpdateProjectdpid(int ps_id, int dp_id)
        {
            Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
            string Connection = Configuration["ConnectionStrings:DefaultConnection"];
           // string Connection = Configuration["ConnectionStrings:DpConnection"];
            ...
            {
                MySqlConnection conn = new MySqlConnection(Connection);
                conn.Execute("update T_Project set dp_id = @dp_id where project_id = @project_id", new { project_id = ps_id, dp_id = dp_id });
                return true;
            }
            catch (Exception ex)
            {
                log.Error("UpdateProjectdp_id Exception :" + ex.Message);
                return false;
            }
        }

        public bool UpdateProjectdownload(int ps_id, string path)
        {
            Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
            string Connection = Configuration["ConnectionStrings:DefaultConnection"];
            // string Connection = Configuration["ConnectionStrings:DpConnection"];
            ...
            {
                MySqlConnection conn = new MySqlConnection(Connection);
                conn.Execute("update T_Project set downloadpath = @downloadpath where project_id = @project_id", new { project_id = ps_id, downloadpath = path });
                return true;
            }
            catch (Exception ex)
            {
                log.Error("UpdateProjectdownload Exception :" + ex.Message);
                return false;
            }
        }
        ...
        public List<Project> getDP_DownloadPath(string project_id)
        {
            List<Project> projectlst = null;
            Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
            string Connection = Configuration["ConnectionStrings:DefaultConnection"];
            ...
            {
                MySqlConnection conn = new MySqlConnection(Connection);
                string sql = "select downloadpath from T_Project as tp where project_id = @Project_id";
                var datas = new Project { Project_id = int.Parse(project_id) };
                projectlst = conn.Query<Project>(sql, datas).ToList();
            }
            catch (Exception ex)
            {
