...
from . import task_export,task_import,task_uid_enc,task_createProject, task_getReport, task_CheckProjStatus
from . import task_import,task_uid_enc,task_createProject, task_setJsonProfile,task_getGenTbl,task_getKchecking, task_DeIdAsyncAES
...
def CancelProjectStatus_Sync():
    ...
    try:
        ...
        response_get = requests.get("http://"+web_ip+":"+web_port+"/api/WebAPI/CancelProjectStatus", params=CancelProject_para)
        response_dic = response_get.json()
...
def AutoCheckProjStatus_Sync():
    ...
    try:
        ...
        response_g = requests.post("http://"+flask_ip+":"+flask_port+"/CheckProjStatus", json=CheckProjStatus_para,timeout=None)
        response_dic = response_g.json()
...
def AutoGetReport_Sync():
    ...
    try:
        ...
        response_g = requests.post("http://"+flask_ip+":"+flask_port+"/getReport_async", json=report_para,timeout=None)
        response_dic = response_g.json()
...
### swagger: AES hashmac 
@app.route("/AESHash_Sync", methods=["POST"])
@swag_from("swagger_yml/AESHash_Sync.yml")
def AESHash_Sync():
    log_time = GetLogTime('AESHash_Sync')
    log_time.printLog("Start AESHash_Sync")
...
### swagger: AES hashmac 
@app.route("/AESDecrypt_Sync", methods=["POST"])
@swag_from("swagger_yml/AESDecrypt_Sync.yml")
def AESDecrypt_Sync():
    log_time = GetLogTime('AESDecrypt_Sync')
    log_time.printLog("Start AESDecrypt_Sync")
...
def JsonProfile_Async():
    ...
    try:
        ...
        response_setJP = requests.post("http://"+flask_ip+":"+flask_port+"/setJP_Async", json=setJP_para,timeout=None)
...
def setAutoDeId_Async():
    ...
    try:
        ...
        response_g = requests.post("http://"+flask_ip+":"+flask_port+"/DeIdAsyncAES", json=AutoDeIdAsync_para,timeout=None, verify=False)
...
#         print('report_para: ', report_para)
#         response_g = requests.post("http://"+flask_ip+":"+flask_port+"/exportData_InterAgent", json=report_para,timeout=None)
#         response_dic = response_g.json()
#         print("report_JSON: ",response_dic)
#         #response['getReport']=response_dic #response_dic['state']
#         response = response_dic
