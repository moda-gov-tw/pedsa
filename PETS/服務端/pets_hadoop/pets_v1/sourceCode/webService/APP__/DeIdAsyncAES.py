...
def main(args):
    ...
    try:
        ...
        response_g = requests.post("http://"+flask_ip+":"+flask_port+"/aes_async", json=Hash_para,timeout=None, verify=False)
    ...
    try: #API:Generalizationasync
        ...
        Gen_para = {"pid":pid, "pname": p_dsname, "selectqivalue":gen_qi_settingvalue, "k_value":minKvalue, "tablename":pro_tb, "r_value":r_value, "t1":t1, "t2":t2}#, "pro_col_en":pro_col_en,"pro_col_cht":pro_col_cht}
    ...
    while int(progress) != 100:
        try: 
            time.sleep(15)
            progress, progress_state  =  check_appstatus(pid, 'getKchecking')
            if int(progress) == 100:
                break;
            if progress_state == 'err':#int(progress) == 10:
