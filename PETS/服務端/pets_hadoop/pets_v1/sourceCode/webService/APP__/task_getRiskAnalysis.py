...
###itri, for deID (start)######################
###itri, for getRisk (start) 202200115 ########
@celery.task(bind=True)

def getRiskAnalysis_longTask(self, _jsonBase64,nothing):
    ...
    with app.app_context():
        ...
        _logger=_getLogger('getRiskAnalysis_longTask')
        _vlogger=_getLogger('getRiskAnalysis_longTask_verify')
        _errlogger  =_getLogger('error__getRisk')
        _vlogger.debug('input : '+_jsonBase64)
        _errlogger.debug("getRiskAnalysis_longTask_error")
        ts0 = time.time()
        #=====================================================

        print ('input : '+_jsonBase64)
        ...
        #     return None
        # try:
        #     df_ori = jsonfile['df_ori'].encode('utf-8')#.encode('utf-8')
        #     df_ano = jsonfile['df_ano'].encode('utf-8')#.encode('utf-8')
        #     p = jsonfile['p'].encode('utf-8')
        #     QI = jsonfile['QI'].encode('utf-8')
        #     SA = jsonfile['SA'].encode('utf-8')
            
        # except Exception as err:
        #     print ('json file first layer error! - %s:%s' %(type(err).__name__, err))
        #     return None

        # print ('json error testing')
        # print ('json part df_ori:' + df_ori)
        # print ('json part df_ano:' + df_ano)
        # print ('json part p:' + p)
        # print ('json part QI:' + QI)
        # print ('json part SA:' + SA)

        # print ('json schema testing 1')
        # schema = getRiskAnalysisSchema()
        # print ('json schema testing 2')
        # data = loadJson(jsonfile, schema)
        # print ('json schema testing 3')
        ...
        # print ('type of SA: ')
        # print(type(SA))
        # print ('json schema testing 6')
        try:

            #20200211 modified
            #sparkCode = getConfig().getSparkCode('risk_core/main.py')
            print("in task_getRiskAnalysis")
            sparkCode = getConfig().getSparkCode('risk_main.py')
            print(sparkCode)
            # cmdStr = '''
            # spark-submit {} '''.format(sparkCode)

            cmdStr = '''
            spark-submit {} {} '''.format(sparkCode, _jsonBase64)
...
def getSparkAppId(self, stdout_, viewSparkProcess_):
    ...
    while True:
        ...
        if "minKvalue" in  line:
            spark_kvalue_index=line.find('minKvalue')
            spark_kvalue_ =line[spark_kvalue_index+10:]
        ...
        if "warningCols:" in line:
            ...
            db = meta_['dbName'].strip('\n')
            tbl = meta_['tblName'].strip('\n')
            colCompare = getColoumnsFromMysql(db, tbl, your_conn)
            if colCompare:
