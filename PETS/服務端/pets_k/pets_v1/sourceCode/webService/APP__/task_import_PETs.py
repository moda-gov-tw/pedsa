...
def import_longTask_PETs(self, base64_,nothing):
    """
    base64_: string
    nothing: 1
    """
...
#            return     
        ...
        MacPath = getConfig().getExportPath('mac')
        # Check if project exists 
        for i in range(5):
            _logger.debug("i:"+str(i))
            d_input_p = os.path.join(serverPath, projName)
            if os.path.exists(d_input_p):
                _logger.debug("d_input_p: " + d_input_p)
                break
            elif i==4:
                break
            else:
                _logger.debug('errTable:projName_does_not_exist: '+d_input_p)
                time.sleep(10)
            # os.popen('mkdir {} '.format(d_input_p))

            # data_mac_output = os.path.join(MacPath,projName)
            # _logger.debug('data_mac_output: '+data_mac_output)

            # os.system('cp -r {} {}'.format(data_mac_output, d_input_p))
        ...
        try:
            ...
            sparkCode = getConfig().getSparkCode('import_PETs.py')
