...
#main(projName,exportDictEncode,path_,userAccount,userId)
def main(projName,exportDictEncode,path_,userAccount,userId):
    ...
    if not re.match(base64_pattern, str(exportDictEncode)):
        _logger.debug("Invalid exportDictEncode format")
        return 'Fail'
    ...
    for tblName in tblInfo:
        ...
        try:
            ...
            # 構建命令
            #cmd = [
            #    'scp', '-o', 'StrictHostKeyChecking=no', '-P', port , '-r',  shlex.quote(os.path.join(path_,projName)),
            #    f'hadoop@{ip}:{out_path}'
            #]
            
            #ip="168.17.8.252"
            #cmd = [
            #    'scp', '-o', 'StrictHostKeyChecking=no',  '-r',  os.path.join(path_,projName),
            #    f'hadoop@{ip}:{out_path}'
            #]
            cmd = 'scp -r -O '+ os.path.join(path_,projName) + ' hadoop@168.17.8.252:' + out_path + ''
