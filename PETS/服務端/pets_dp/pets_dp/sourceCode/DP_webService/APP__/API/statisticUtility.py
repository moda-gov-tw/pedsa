...
def get_obcol(conn,projID):
    sqlStr = f"SELECT ob_col FROM DpService.T_ProjectColumnType WHERE project_id = '{projID}'"
...
#################################################
    ...
    try:
        check_conn = ConnectSQL()
        #updateToMysql_status(check_conn, projID, projName, fileName, step, percentage)
        updateToMysql_T_ProjectStatus(check_conn,projID,10,"資料相似度報表")
