...
from . import task_PETs_preview, task_PETs_exportData, task_PETs_MLutility, task_PETs_DP, task_PETs_DP_data
...
@app.route('/DP_async', methods=['POST'])
def DP_async():
    print("Start  DP_corr_async")
    ts0 = time.time()
    app_ID=99999
    try:
        data_raw = request.get_json()
    ...
    task = task_PETs_DP.DP_longTask.apply_async((jsonBase64__,1)) #1224:pei
    #############################################################################
    ...
    if True:
        ...
        while state_ != 'PROGRESS':
            #####20181121, citc trace#####################################################
            genData_longTask_task = task_PETs_DP.DP_longTask.AsyncResult(task.id)
            #########################################################################
            state_ = genData_longTask_task.state
            print (state_)
            #response['state'] = state_
...
@app.route('/DP_data_async', methods=['POST'])
def DP_data_async():
    print("Start GenerationDPData_async")
    ts0 = time.time()
    app_ID=99999
    try:
        data_raw = request.get_json()
    ...
    task = task_PETs_DP_data.DP_data_longTask.apply_async((jsonBase64__,1)) #1224:pei
    #############################################################################
    ...
    if True:
        ...
        while state_ != 'PROGRESS':
            #####20181121, citc trace#####################################################
            genData_longTask_task = task_PETs_DP_data.DP_data_longTask.AsyncResult(task.id)
            #########################################################################
            state_ = genData_longTask_task.state
            print (state_)
            #response['state'] = state_
