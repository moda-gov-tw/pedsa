...
def PETs_MLutility_longTask(self, _jsonBase64,nothing):
    ...
    try:
        ...
        privacyType = jsonfile['privacyType']#.encode('utf-8')
        ...
        if not isinstance(targetCols, list):
            _logger.debug("Input should be a list.")
            return 'Fail'
        
        valid_projIDs = ["dp", "k", "syn"]
        if privacyType not in valid_projIDs:
            _logger.debug("Invalid privacyType format")
            return 'Fail'
    ...
    cmd='python /app/app/devp/API/PETs_MLutility.py {} {} {} {} \"{}\"'.format(userID, projID, projName, privacyType, str(targetCols))
    _vlogger.debug(cmd)
    ...
    #        try:
    #            check_conn = ConnectSQL()
    #            updateToMysql_utilityResult(self, check_conn, projID, target_col, privacyType, model, MLresult_b64)
...
def updateToMysql_utilityResult(self, conn, projID, targetCol, privacy_type, model, MLresult):
    # update process status to mysql
