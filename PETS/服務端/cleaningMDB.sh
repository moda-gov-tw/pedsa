...
docker_result_str=$(docker cp ./DpService_dump.sql $MariaDB_nrt_ID:/)
echo $docker_result_str

docker_result_str=$(docker cp ./PetsService_dump.sql $MariaDB_nrt_ID:/)
echo $docker_result_str
...
docker_result_str=$(docker exec -i $MariaDB_nrt_ID /bin/bash -c "mysql --default-character-set=utf8 -u root -pcitcw200 </syn_db.sql")
echo $docker_result_str

docker_result_str=$(docker exec -i $MariaDB_nrt_ID /bin/bash -c "mysql -f --default-character-set=utf8 -u root -pcitcw200 </PetsService_dump.sql")
echo $docker_result_str

docker_result_str=$(docker exec -i $MariaDB_nrt_ID /bin/bash -c "mysql -f --default-character-set=utf8 -u root -pcitcw200 </DpService_dump.sql")
echo $docker_result_str
#echo "leave --cleaning MariaDB_nrt-- "
# mysql --default-character-set=utf8 -u root -pcitcw200 </key_db1.sql
