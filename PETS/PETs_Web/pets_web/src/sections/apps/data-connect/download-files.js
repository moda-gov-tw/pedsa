 ...
 * @returns {JSX.Element}
 */
export default function DownloadFiles({ height = 200, argArray = [{ 'button_name': '', 'file_name': '', 'url': '', 'icon': AttachFileIcon, 'behavior': 'download', 'clickFunc': null, 'disabled': false }], listItemComponent = Paper }) {
    ...
    function renderRow(props) {
        ...
        return (
            <ListItem style={style} key={index} component={listItemComponent} elevation={1} disablePadding sx={{ borderRadius: 0 }}>
                <ListItemButton disabled={argArray[index].disabled} onClick={() => handleClick(file_info_array[index].url, file_info_array[index].file_name, file_info_array[index].behavior, file_info_array[index].clickFunc)}>
                    {/* Icons */}
                    <ListItemIcon>
                        {file_info_array[index].icon == null ? <AttachFileIcon sx={{ fontSize: 20, transform: 'rotate(45deg)' }} /> : file_info_array[index].icon}
                    </ListItemIcon>
                    {/* Button Text */}
    ...
    return (
        <>
            ...
            <FixedSizeList
                ...
                width={'80%'}
                itemSize={40}
                itemCount={file_info_array.length}
                overscanCount={5}
            >
                {renderRow}
            </FixedSizeList>
