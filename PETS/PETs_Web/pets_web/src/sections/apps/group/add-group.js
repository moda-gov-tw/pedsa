...
const AddGroup = ({ group, groupId, allGroups, setAllGroups, onCancel }) => {
  ...
  async function handleCreateGroup() {
      await axios.post('/api/group/create_group/', {
          ...
          'project_quota': values.project_quota,
        },
        {
          headers: {
          ...
          .catch(async (error) => {
            await setPopUpMessage('新增單位失敗，請確認單位是否已存在');
            if (!wroteLog["addGroup"]) {
  ...
  async function handleEditGroup() {
    ...
    newGroupData,
    {
      headers: {
        ...
        .catch(async (error) => {
          await setPopUpMessage('編輯單位失敗，請確認單位是否已存在');
          if (!wroteLog["editGroup"]) {
                    ...
                    <Grid item xs={12}>
                      <Stack spacing={1.25}>
                        ...
                        <TextField
                          ...
                          disabled={!!group}
                          label={checkGroupType(values.group_type) && '不可包含中文字元及任何特殊字元，且字數上限為3'}
