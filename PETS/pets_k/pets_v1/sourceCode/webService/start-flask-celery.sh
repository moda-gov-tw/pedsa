...
#
echo "start redis..."
docker run -d -ti --name redis -v redis-data:/data -v /etc/localtime:/etc/localtime:ro -p 6380:6379 -p 3320:22 --network hadoopnet --ip 172.28.1.2 redis:4.0.1-alpine02 redis-server --requirepass 'citcw200'
...
docker run -d -ti --name flask -e REDIS_HOST=redis -e REDIS_PORT=6379 -e REDIS_PWD=citcw200 \
       ...
       --network hadoopnet -p 5088:5088 -p 3321:22 --ip 172.28.1.3 --hostname flask.bdp.com flask-redis_citc:04 \
...
docker run -d -ti --name deidweb --network hadoopnet --ip 172.28.1.10 -p 11000:11000 deidwebssd:1.2.71
#sleep 5
