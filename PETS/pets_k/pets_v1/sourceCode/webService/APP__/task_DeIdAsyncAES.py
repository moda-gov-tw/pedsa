...
###2020/07/08 非同步自動化 DeIdAsync ######################
@celery.task(bind=True)
def DeIdAsync_longTask(self, _jsonBase64,nothing):
    ...
    # _errlogger=_getLogger('verify_DeIdAsync_longTask')
    _vlogger=_getLogger('verify_DeIdAsync')
    _errlogger  =_getLogger('error__DeIdAsync')
    ...
    if jsonfile is None:
        ...
        self.update_state(state="FAIL", meta={'errMsg':errMsg,'state_no':'-1'}) 
        return 'Fail'
    ...
    try:
        ...
        onlyHash = "N"
        memberid = jsonfile['memberid']
        memberacc = jsonfile['memberacc']
    ...
    if pname == '' or prodesc == '' or powner == '' or p_dsname == '' or hashTableName == '' or hashkey == '' or sep == '' or columns_mac == '' or dataHash == '' or configName == '':
        errMsg = 'Parameter is None!'
        self.update_state(state="FAIL", meta={'errMsg':errMsg,'stateno':'-1'})
        return 'Fail'

    print('pname:{}, prodesc:{}, powner:{}, p_dsname:{}, hashTableName:{}, hashkey:{}, sep:{}, columns_mac:{}, dataHash:{}, configName:{}').format(pname, prodesc, powner, p_dsname, hashTableName, hashkey, sep, columns_mac, dataHash, configName)
    ...
    #     # InsertProject_para = { "p_dsname": p_dsname, "pname": pname, "powner": powner, "prodesc": prodesc } "-memberid",memberid,"-memberacc",memberacc
    #             # InsertProject_para = { "p_dsname": p_dsname, "pname": pname, "powner": powner, "prodesc": prodesc }  #原版
    #     # InsertProject_para = { "p_dsname": pname, "pname": p_dsname, "powner": powner, "prodesc": prodesc ,'memberid': memberid,'memberacc': memberacc}
    #     response_g = requests.post("https://"+flask_ip+":"+flask_port+"/aes_async", json=Hash_para,timeout=None, verify=False)
    ...
    #     response['HashMac']='1'
    # except Exception as e:
    #     _errlogger.debug('HASH_request error. {0}'.format(str(e)))

    try:
        # InsertProject_para = { "p_dsname": p_dsname, "pname": pname, "powner": powner, "prodesc": prodesc } "-memberid",memberid,"-memberacc",memberacc
                # InsertProject_para = { "p_dsname": p_dsname, "pname": pname, "powner": powner, "prodesc": prodesc }  #原版
        InsertProject_para = { "p_dsname": pname, "pname": p_dsname, "powner": powner, "prodesc": prodesc ,'memberid': memberid,'memberacc': memberacc}
        _vlogger.debug('web_ip: '+ str(web_ip))
        _vlogger.debug('web_port: '+ str(web_port))
        ...
        _errlogger.debug('InsertProject_request error. {0}'.format(str(e)))
    ...
    try:
        if int(response['InsertProject_flag'])==1:
            ...
            errMsg = 'InsertProject fail: - %s' %(error_insertproj)
            _errlogger.debug('errTable: {0}'.format(str(errMsg)))
            self.update_state(state='FAIL', meta={'errMsg':errMsg,'stateno':'-1'})
            return {'errMsg':errMsg,'stateno':'-1'}
            sys.exit(0)
        else:
        ...
        _errlogger.debug('errTable: InsertProject error. {0}'.format(str(e)))
        errMsg = 'errTable: InsertProject error. {0}'.format(str(e))
        return {'errMsg':errMsg,'stateno':'-1'}
        sys.exit(0)

         
    # print cmdStr
    cmd=["python","app/devp/DeIdAsyncAES.py","-pname",pname,"-prodesc",prodesc,"-powner",powner,"-p_dsname",p_dsname,"-step",step,"-configName",configName,"-hashTableName",hashTableName,"-hashkey",hashkey,"-sep",sep,"-columns_mac",columns_mac,"-dataHash",dataHash,"-onlyHash",onlyHash,"-memberid",memberid,"-memberacc",memberacc]
    print(cmd)
