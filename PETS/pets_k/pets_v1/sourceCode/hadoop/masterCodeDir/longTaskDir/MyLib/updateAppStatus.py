...
class updateAppStatus:
    ...
    def __init__(self,appID_, appName_, dbName_, projId_=999, userId_=1):
        self.appID=appID_
    ...
    def updateToMysql(self,appState, progress,progress_state="Running"):
        ...
        if resultSampleData['result'] == 1:
            ...
            print("(updateAppStatus)Update mysql succeed. {0}".format(resultSampleData['msg']))
            self.conn.close()
            return True
        else:
            msg = resultSampleData['msg']
            #_logger.debug('insertSampleDataToMysql fail: ' + msg)
            print('(updateAppStatus) insertNodeStatusToMysql fail: ' + msg)
            #self.conn.close()
            return False
