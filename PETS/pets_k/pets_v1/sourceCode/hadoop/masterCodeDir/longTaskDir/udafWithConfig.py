    ...
    for udf_ in udfs:
        try:
            if udf_ == 'getGenAddress':
                sqlAction = "create temporary function " + udf_ + "_ as 'citc.udfEncrypt." + udf_ + "'"
            else:
                sqlAction = "create temporary function " + udf_ + "_ as 'citc.udfEncrypt." + udf_ + "'"
            sqlAction = "CREATE TEMPORARY FUNCTION contains as 'citc.udfEncrypt.ComplexUDFExample'"
...
def main(dbName, tableName, sepFormat):
    ...
    try:
        genAction = "select name, contains(name_list,name_list[0]), name_list from default.peoplelist"
