...
#        _logger.debug(sc.applicationId)
    ...
    try:
        ...
        updateAppStatus_ = updateAppStatus(sc.applicationId, appName,db_name,projID)
    except Exception as e:
...
#        print(dfK_finish.count())
        ...
        _vlogger.debug("distinct_count:%i" %(dc))
        _logger.debug(warningCols)
        _logger.debug("warningCols:%s" % (warningColsStr))
...
#        print(put)
        ...
        ####################################################################################
        registerRealHiveTable_forsparksql(dfK_finish, final_tb_name)
        ####################################################################################
...
#registerRealHiveTable_forsparksql(maskkValueDF_, "Ntu"+"_"+jobName+"_"+projStep+"_"+put)
        ...
        s = e.java_exception.toString()
        _logger.debug(s)
        updateAppStatus_.updateToMysql("Py4JJavaError" ,"100","err")
        ...
        _logger.debug(len(sys.exc_info()))
        _logger.debug("errTable_errReadFromHive")
        updateAppStatus_.updateToMysql("errTable_errReadFromHive" ,"100","err")
        updateTProjectStatus_.updateToMysql(project_id, 97,"error")
    ...
    try:
        ...
        updateAppStatus_.updateToMysql("errTable: DeID preview fai" ,"100","err")
        updateTProjectStatus_.updateToMysql(project_id, 97,"error")
