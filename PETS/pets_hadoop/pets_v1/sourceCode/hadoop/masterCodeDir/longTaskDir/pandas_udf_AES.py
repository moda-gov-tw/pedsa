...
#https://www.cnblogs.com/wkang/p/10255043.html
#https://hyukjin-spark.readthedocs.io/en/latest/reference/api/pyspark.sql.functions.pandas_udf.html
####################################################################################
...
#20201230, citc add
def aes_cbc_encrypt_b64(key, iv, data):
    secret_key = key.encode('utf-8')
    iv = iv.encode('utf-8')
    data = data.encode('utf-8')

    # pad data algorithms.AES.block_size=128 bit
...
def aes_cbc_decrypt_b64(key, iv, data_b64):
    secret_key = key.encode('utf-8')
    iv = iv.encode('utf-8')
    #print(repr(data_b64))
    value = base64.b64decode(data_b64)
    #print(repr(value))
    cipher = Cipher(
            algorithms.AES(secret_key),
            modes.CBC(iv),
...
def udfMacCols(encList, df, key_, idHashDBName, headerDict):
    ...
    ###citc add for pandas_udf,####################################
    key=key_#"Fubon1234561Fubon1234561Fubon123"
    #print(len(key))
    iv = "citc__InitVector"
    @pandas_udf("string", PandasUDFType.SCALAR)
    def udfMac__(data):
        #both input/output are all pandas.Series of string
        #get input string from data (a pandas.Series with only one element)
        data = data[0]
        print('2##################key={}, type(key)={}'.format(key, type(key)))
        encrypted = aes_cbc_encrypt_b64(key, iv, data) 
        print('3##################encrypted={}, type(encrypted)={}'.format(encrypted, type(encrypted)))
        #output must ba a pandas.Series with only one element=encrypted.decode('utf-8')
        #encrypted.decode, form byte string to string
        return pd.Series(encrypted)

    #sqlContext.sql("create temporary function udfMac_ as 'citc.udfEncrypt.udfEnc'")
    spark.udf.register("udfMac_", udfMac__)
    ...
    for colNam_ in cols:
        if colNam_ in cols[-1:]:
            if colNam_ not in encList:
                ...
                tmpStr=tmpStr+ colNam_ + " as "+colNam_+"__, "+"udfMac_("+colNam_+") as "+colNam_+" from DF_Table__"
            break;
        if colNam_ not in encList:
            tmpStr=tmpStr+ colNam_+","
        else:
            tmpStr=tmpStr+ colNam_ + " as "+colNam_+"__, "+"udfMac_("+colNam_+") as "+colNam_+","
    print (tmpStr)
...
def main__(tblName, key_, colsNum, totalLen):
    ...
    spark_.conf.set("spark.sql.execution.arrow.pyspark.enabled", "true")
    ...
    try: 
        ...
        if 0:
            ...
            #citc, 20200815 add
            _logger.debug('befor exportData============')
            #dfDF.show()
            my_list = dfDF.select("col_0").rdd.flatMap(lambda x: x).collect()
            #_logger.debug('my_list[0]============')
            #_logger.debug(my_list[0])
            #_logger.debug('my_list[0]====end========')
