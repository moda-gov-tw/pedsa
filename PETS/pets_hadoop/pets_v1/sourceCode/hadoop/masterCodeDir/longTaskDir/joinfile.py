...
#########################################
    ...
    #ssh_check_command = f'sshpass -p citcw200@ ssh hadoop@{web_ip} "[ -d \\"/home/hadoop/proj_/data/input/{projName}\\" ]"'
    #runcode = os.system(ssh_check_command)

    #if runcode == 0:
    #    ssh_remove_command = f'sshpass -p citcw200@ ssh hadoop@{web_ip} "rm -r /home/hadoop/proj_/data/input/{projName}"'
    #    runcode = os.system(ssh_remove_command)

    #sshpass -p citcw200@ ssh -o StrictHostKeyChecking=no -p 5922 hadoop@34.81.71.21 "rm -rf /home/hadoop/proj_/data/input/demo_1128_3"
    ...
    try:  
        ...
        response_dp = requests.get("http://"+gan_ip+":"+dp_port+"/api/WebAPI/dp_conn", params=dp_conn_para,timeout=None, verify=False)
