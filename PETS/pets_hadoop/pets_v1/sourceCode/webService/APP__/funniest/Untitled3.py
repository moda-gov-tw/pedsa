...
class HiveLibs(object):
	...
	def __init__(self, SparkContext_):
        #sc = SparkContext()
		self._logger=_getLogger(self.__class__.__name__)
		self.sqlContext=HiveContext(SparkContext_)
		self.join_ = self.Join()
		self.kChking=self.K()
		self.generalization=self.Generalization()
		self.dbOperation=self.DbOperation(self.sqlContext,self._logger)
	...
	class Generalization():
		...
		#20170515	
			...
			def getColumnWithRmQuatTrim(self, col_name_,tmp_tb_name_):
				...
				df____=ddf.replace(['\N'],['NA'],col_name_).replace(['na'],['NA'],col_name_).replace([''],['NULL'],col_name_)
			...
			def getIndirectColumns_NoPreprocess(self, list_,df):
				...
				for col_name_ in list_:
					#ddf=ddf.replace(['\N'],['NA'],col_name_).replace(['na'],['NA'],col_name_).replace([''],['NULL'],col_name_)
					ddf=ddf.replace(['"\\N"'],['"NA"'],col_name_).replace(['"\N"'],['"NA"'],col_name_).replace(['"na"'],['"NA"'],col_name_).replace(['""'],['"NULL"'],col_name_)
			...
			def getOutlierList(self, col_name_,tmp_tb_name_, n):
				...
				df__=ddf.replace(['\N'],['NA'],col_name_).replace(['na'],['NA'],col_name_).replace([''],['NULL'],col_name_)
