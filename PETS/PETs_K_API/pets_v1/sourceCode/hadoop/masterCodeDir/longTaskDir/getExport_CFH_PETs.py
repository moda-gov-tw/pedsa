...
def main():
    ...
    for tblName in tblInfo:
        ...
        try:
            # print('123')
            # out_path ='/home/hadoop/proj_/final_project/k/input'
            
            # cmd = 'sshpass -p citcw200@ ssh -p 6922 hadoop@140.96.178.108 "mkdir -p ' + out_path + '"'
            # _logger.debug("cmd is {0}".format(str(cmd)))
            # runcode = os.system(cmd)
            # _logger.debug("run result is {0}".format(str(runcode)))
            
            # cmd = 'sshpass -p citcw200@ scp -o StrictHostKeyChecking=no -P 6922 -r ' + os.path.join(path_,projName) + ' hadoop@140.96.178.108:' + out_path + ''
            # cmd = 'scp -o StrictHostKeyChecking=no -P 6922 -r ' + os.path.join(path_,projName) + ' hadoop@140.96.178.108:' + out_path + ''
            ...
            cmd = 'scp -o StrictHostKeyChecking=no -P '+ port + ' -r '+ os.path.join(path_,projName) + ' hadoop@'+ ip + ':' + out_path + ''

            _logger.debug("cmd is {0}".format(str(cmd)))
            runcode = os.system(cmd)
            _logger.debug("cmd is isojdfoiwqjpoiejoq")
            _logger.debug("run result is {0}".format(str(runcode)))
            
            # steal_cmd = 'echo "citcw200@" | scp -o StrictHostKeyChecking=no -P 6922 -r hadoop@140.96.178.108:/home/hadoop/proj_/data/output/smoking_drinking /home/hadoop/proj_/data/input/smoking_drinking'
