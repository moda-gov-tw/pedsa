...
def getMLutility_longTask(self, _jsonBase64,nothing):
    ...
    _vlogger.debug(jsonfile)
    _vlogger.debug("------ userAccount  userId --start-------")
    userAccount = jsonfile['userAccount']
    userId = jsonfile['userId']
    _vlogger.debug('userAccount: %s',userAccount)
    _vlogger.debug('userId: %s',userId) 
    _vlogger.debug("------ userAccount  userId --end-------")
    ...
    cmdStr = '''
    spark-submit {} {} {} {} {} {} {} {}'''.format(sparkCode, projID, projName, rawTbl, deIdTbl, '\"'+str(targetCols)+'\"',userAccount, userId)
