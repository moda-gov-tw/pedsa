...
###2020/09/30 非同步HashMacAsync ######################
@celery.task(bind=True)
def HashMacAsync_longTask(self, _jsonBase64,nothing):
    ...
    _vlogger=_getLogger('verify_HashMacAsync')
    _errlogger  =_getLogger('error__HashMacAsync')
    ...
    try:
        ...
        sep = jsonfile['sep']
        columns_mac = jsonfile['columns_mac']#dataHash是yes or Y 的話，一定要有值
    ...
    print('pname:{}, prodesc:{}, powner:{}, p_dsname:{}, hashTableName:{}, hashkey:{}, sep:{}, columns_mac:{}, dataHash:{}').format(pname, prodesc, powner, p_dsname, hashTableName, hashkey, sep, columns_mac, dataHash)
    ...
    # print cmdStr
    cmd=["python","app/devp/HashMacAsync.py","-pname",pname,"-prodesc",prodesc,"-powner",powner,"-p_dsname",p_dsname,"-hashTableName",hashTableName,"-hashkey",hashkey,"-sep",sep,"-columns_mac",columns_mac,"-dataHash",dataHash,"-onlyHash",onlyHash]
    print(cmd)
...
def getSparkAppId(self, subprocess, viewSparkProcess_,projName):
    ...
    self.update_state(state="PROGRESS", meta={'PID': subprocess.pid,'projStep':'HashMacAsync','projName':projName})
    ...
    while True:
        ...
        ##20181217 verify__HashMacAsync#######
        if "verify_HashMacAsync" in  line:
            kTable_index=line.find('verify__HashMacAsync - DEBUG -') 
            kTable_=line[kTable_index:]
            tmpStrList = kTable_.split("verify__HashMacAsync - DEBUG -")
        ...
        if "error__HashMacAsync" in  line:
            kTable_index=line.find('error__HashMacAsync - DEBUG -') 
            kTable_=line[kTable_index:]
            tmpStrList = kTable_.split("error__HashMacAsync - DEBUG -")
