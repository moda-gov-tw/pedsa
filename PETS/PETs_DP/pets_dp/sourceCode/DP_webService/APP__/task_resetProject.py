...
###itri, for resetProject(start)######################
@celery.task(bind=True)
# remove project folder to record the synthetic data generation process
# under the folderForSynthetic/
def resetProject_longTask(self, _jsonBase64,nothing):

    #regist logger in log/logging_setting.yaml
    global _logger, _vlogger, _errlogger
    _logger=_getLogger('verify_resetProject_longTask')
    _vlogger=_getLogger('verify_resetProject')
    _errlogger  =_getLogger('error__resetProject')
    ...
    cmd=["python","app/devp/API/resetProject.py","-projName",projName]
    print(cmd)
...
def getSparkAppId(self, subprocess, viewSparkProcess_,projID,projName,userID):
    ...
    meta_={'PID': subprocess.pid,'userID':userID,'projID': projID,'projStep':'Reset Project','projName':projName}
    ...
    while True:
        ...
        ##filter log in logger:verify__resetProject#######
        if "verify__resetProject" in  line:
            kTable_index=line.find('verify__resetProject - DEBUG -') 
            kTable_=line[kTable_index:]
            tmpStrList = kTable_.split("verify__resetProject - DEBUG -")
        ...
        ##filter error log in logger:error__resetProject#######     
        if "error__resetProject" in  line:
            kTable_index=line.find('error__resetProject - DEBUG -') 
            kTable_=line[kTable_index:]
            tmpStrList = kTable_.split("error__resetProject - DEBUG -")
            _errlogger.debug(tmpStrList[1])
            kTable_ = tmpStrList[1][0:len(tmpStrList[1])-1]
            print('error__resetProject: ' + tmpStrList[1])
            print ('task id is '+self.request.id)
            meta_['Msg'] = kTable_
            outList.append(kTable_)
