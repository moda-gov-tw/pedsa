...
def sampling(directory,sum_col_num,epoch=120):
    ...
    #print("epoch: ",epoch)
    original = pd.read_csv(directory+'pkl/test_category_conti.csv',header=0, encoding='utf-8')
    ...
    #print(df.shape)
    df.to_csv(directory+'sample/synthetic_sample_'+str(epoch)+'.csv',float_format='%.7f',index=False,header=False, encoding='utf-8')
...
def recovering_syn(directory,sum_col_num,col_name,conti_col,e):
    dir_syn=(directory+'transform/')
    if not os.path.exists(dir_syn):
        os.mkdir(dir_syn)
    fake_data = pd.read_csv(directory+"sample/synthetic_sample_"+str(e)+'.csv', header=None)
    class_embedding = []
...
#     print(recover_fake_data.head())
    recover_fake_data.to_csv(directory+'transform/synthetic_transform_'+str(e)+'.csv',index=False,header=True, encoding='utf-8')    
