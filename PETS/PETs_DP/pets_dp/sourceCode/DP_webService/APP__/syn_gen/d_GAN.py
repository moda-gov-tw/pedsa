...
def pre_proccesing(directory,sum_col):
    ...
    dataset=pd.read_csv(directory+'pkl/test_category_conti.csv',header=0)
    #print(dataset.head())
...
#         os.mkdir(dir_)
    dir_syn=(directory+'synthetic')
    if not os.path.exists(dir_syn):
        os.mkdir(dir_syn)
    dir_loss=(directory+'loss')
    if not os.path.exists(dir_loss):
        os.mkdir(dir_loss)
    dir_model=(directory+'models')
    ...
    generator.add(Dense(randomDim, input_dim=randomDim,kernel_initializer=initializers.random_normal(stddev=0.5)))# init=initNormal))
    generator.add(LeakyReLU())
    #generator.add(Activation('relu'))
    generator.add(Dense(randomDim*2))
    generator.add(LeakyReLU())
    ...
    discriminator.add(Dense(randomDim*2, input_dim=randomDim,kernel_initializer=initializers.random_normal(stddev=0.5)))# init=initNormal))
    discriminator.add(LeakyReLU())
    discriminator.add(Dropout(0.3))
    discriminator.add(Dense(randomDim))
    discriminator.add(LeakyReLU())
...
#                 train_start_time = time.clock()
                ...
                for _ in range(int(batchCount)):
                    ...
                    noise = np.random.normal(0, 0.5, (batchSize, randomDim))
                    ...
                    # Train generator
                    noise = np.random.normal(0, 0.5, size=[batchSize, randomDim])
